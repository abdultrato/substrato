...
from domain.clinical.regras_paciente import InterpretadorResultado as InterpretadorReferencia
...
class ServicoResultado:
    ...
    def _delta_check(resultado_item):
        ...
        anterior = (
            resultado_item.__class__.objects.filter(
                resultado__requisicao__paciente=paciente,
                exame_campo=campo,
            )
            .exclude(pk=resultado_item.pk)
            .order_by("-criado_em")
            .first()
        )
