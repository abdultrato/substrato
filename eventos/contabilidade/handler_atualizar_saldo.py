...
from eventos.contabilidade.ledger_entry_criado import LedgerEntryCriado
...
def handle(
	...
	saldos = SaldoConta.objects.select_for_update().filter(
	...
	for conta_id in agregados.keys():
		if conta_id not in saldos_dict:
			SaldoConta.objects.create(
					conta_id = conta_id,
					saldo_atual = 0,
					total_debitos = 0,
					total_creditos = 0,
