...
class Lancamento(
	...
	def confirmar(
		...
		# 🔒 Lock pessimista
		lancamento = (
				Lancamento.objects.select_for_update()
				.prefetch_related(
						"movimentos",
						)
				.get(
		...
		if lancamento.movimentos.count() < 2:
			raise ValidationError(
					"Lançamento deve possuir pelo menos 2 movimentos.",
					)
	...
	def save(
		...
		if self.pk:
			original = Lancamento.objects.get(
					pk = self.pk,
					)
			
			if original.confirmado:
				raise RuntimeError(
						"Lancamento confirmado é imutável.",
						)
	...
	def delete(
		...
		raise RuntimeError(
				"Lancamento não pode ser removido.",
				)
