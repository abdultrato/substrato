...
class Conta(CoreModel, ) :
	...
	class Meta :
		indexes = [models.Index(fields = ["inquilino", "id_custom", ], ), models.Index(fields = ["inquilino", "tipo", ], ), ]
		
		constraints = [models.UniqueConstraint(fields = ["inquilino", "id_custom", ], condition = Q(deletado = False, ), name = "unique_codigo_conta_por_inquilino", ), ]
	
	# 🔒 Impedir alteração estrutural após uso
	def save(self, *args, **kwargs, ) :
		if self.pk :
			original = Conta.objects.get(pk = self.pk, )
			
			if original.tipo != self.tipo :
				if self.tem_movimentacao() :
					raise ValidationError("Não é permitido alterar o tipo de uma conta já "
					                      "utilizada.", )
	...
	def tem_movimentacao(self, ) :
		...
		return LedgerLine.objects.filter(conta_id = self.pk, ).exists()
