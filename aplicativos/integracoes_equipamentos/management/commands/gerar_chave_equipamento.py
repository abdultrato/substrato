...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        equipamento = IntegracaoEquipamento.objects.filter(id_custom=equipamento_id_custom, deletado=False).first()
        if equipamento is None:
            raise CommandError("Equipamento não encontrado.")

        cred, chave = IntegracaoCredencial.gerar(equipamento=equipamento, label=label)
