...
def ensure_produtos(n: int, tenants: list[Inquilino], categorias: list[CategoriaProduto], faker: Faker) -> list[Produto]:
...
def ensure_itens_venda(n: int, vendas: list[Venda], produtos: list[Produto]) -> list[ItemVenda]:
	...
	while _count(ItemVenda) < n:
		...
		produto = next(
			(p for p in produtos_tenant if not ItemVenda.objects.filter(venda=venda, produto=p, deletado=False).exists()),
			None,
		)
		if produto is None:
			continue
...
def ensure_requisicao_itens(n: int, requisicoes: list[RequisicaoAnalise], exames: list[Exame], exames_medicos: list[ExameMedico]) -> None:
...
def ensure_catalogo_materiais(n: int, catalogos: list[ProcedimentoCatalogo], produtos: list[Produto], faker: Faker) -> None:
...
def ensure_procedimento_itens(n: int, procedimentos: list[Procedimento], catalogos: list[ProcedimentoCatalogo]) -> list[ProcedimentoItem]:
...
def ensure_faturas(n: int, requisicoes: list[RequisicaoAnalise], vendas: list[Venda], procedimentos: list[Procedimento]) -> list[Fatura]:
...
def ensure_checkins(n: int, pacientes: list[Paciente], users: list[Usuario], requisicoes: list[RequisicaoAnalise], faturas: list[Fatura], faker: Faker) -> None:
...
def report(minimo: int) -> None:
	print("=== CONTAGEM FINAL (seed_demo) ===")
	faltando: list[str] = []
	app_configs = sorted(
	...
	if faltando:
		...
		print(f"OK: todos os modelos possuem pelo menos {minimo} registros.")
...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		itens_venda = ensure_itens_venda(n, vendas, produtos)
