...
class ExameCampo(CoreModel) :
	...
	exame = models.ForeignKey(Exame, on_delete = models.CASCADE, related_name = "campos", db_index = True, )
	
	tipo = TipoResultadoField(db_index = True)
	unidade = UnidadeField(choices = UnidadePadrao.choices, default = UnidadePadrao.P_UL, verbose_name = "Unidade", )
	...
	class Meta :
		...
		indexes = [models.Index(fields = ["exame"]), models.Index(fields = ["tipo"]), models.Index(fields = ["exame", "ativo", "deletado"]), ]
		
		constraints = [models.UniqueConstraint(fields = ["exame", "nome"], condition = Q(deletado = False), name = "unique_nome_por_exame_ativo", ), ]
	...
	def clean(self) :
		...
		if tipo == TipoResultado.NUMERICO :
			if self.valor_minimo is None and self.valor_maximo is None :
				raise ValidationError({"valor_minimo" : "Defina valores de referência.", "valor_maximo" : "Defina valores de referência.", })
			
			if self.valor_minimo is not None and self.valor_maximo is not None and self.valor_minimo > self.valor_maximo :
				raise ValidationError({"valor_minimo" : "Não pode ser maior que o máximo."})
			
			if self.critico_baixo is not None and self.valor_minimo is not None and self.critico_baixo > self.valor_minimo :
				raise ValidationError({"critico_baixo" : "Deve ser menor que o mínimo."})
			
			if self.critico_alto is not None and self.valor_maximo is not None and self.critico_alto < self.valor_maximo :
			...
			if not self.valores_normais :
				raise ValidationError({"valores_normais" : "Defina os valores considerados normais.", })
		
		elif tipo in (TipoResultado.SEMIQUANTITATIVO, TipoResultado.TEXTO,) :
			possui_referencias_numericas = any(valor is not None for valor in (self.valor_minimo, self.valor_maximo, self.critico_baixo, self.critico_alto,))
			
			possui_referencias_qualitativas = any(valor is not None for valor in (self.valores_normais, self.valores_alterados, self.valores_criticos,))
			
			if possui_referencias_numericas or possui_referencias_qualitativas :
				raise ValidationError("Este tipo não deve possuir referências clínicas.")
