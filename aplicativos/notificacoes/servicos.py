...
CANAIS = {
    Notificacao.Canal.EMAIL: CanalEmail(),
    Notificacao.Canal.SMS: CanalSMS(),
    Notificacao.Canal.WHATSAPP: CanalWhatsApp(),
}


class ServicoNotificacao:
    def _canal_ativo(self, canal):
        if canal == Notificacao.Canal.EMAIL:
            ativo = getattr(settings, "NOTIFICACOES_EMAIL_ATIVAS", True)
            return (ativo, "Canal de e-mail desativado por configuração.")

        if canal == Notificacao.Canal.SMS:
        ...
        if canal == Notificacao.Canal.WHATSAPP:
