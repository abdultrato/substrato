# Generated by Django 4.2.29 on 2026-06-12 16:08

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('inquilinos', '0003_subscriptioncharge_subscriptioninvoice_and_more'),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('clinical', '0015_alter_labexamfield_unit'),
    ]

    operations = [
        migrations.AddField(
            model_name='labrequest',
            name='is_occupational',
            field=models.BooleanField(db_column='is_occupational', db_index=True, default=False, help_text='Indica que a requisição se destina a medicina ocupacional.', verbose_name='Requisição de exames ocupacionais'),
        ),
        migrations.CreateModel(
            name='OccupationalExamProfile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created_at', models.DateTimeField(auto_now_add=True, db_column='created_at', db_index=True)),
                ('updated_at', models.DateTimeField(auto_now=True, db_column='updated_at')),
                ('custom_id', models.CharField(blank=True, db_column='custom_id', db_index=True, editable=False, max_length=40, null=True, unique=True)),
                ('deleted', models.BooleanField(db_column='deleted', db_index=True, default=False)),
                ('deleted_at', models.DateTimeField(blank=True, db_column='deleted_at', null=True)),
                ('version', models.PositiveIntegerField(db_column='version', default=1)),
                ('name', models.CharField(db_column='name', db_index=True, max_length=120)),
                ('profession', models.CharField(blank=True, db_column='profession', db_index=True, default='', help_text='Profissão a que o perfil se destina (ex.: motorista, cozinheiro).', max_length=150, verbose_name='Profissão')),
                ('description', models.TextField(blank=True, db_column='description', default='', help_text='Notas sobre o âmbito do perfil ocupacional.', verbose_name='Descrição')),
                ('active', models.BooleanField(db_column='active', db_index=True, default=True, verbose_name='Ativo')),
                ('created_by', models.ForeignKey(blank=True, db_column='created_by_id', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='%(class)s_criado', to=settings.AUTH_USER_MODEL)),
                ('deleted_by', models.ForeignKey(blank=True, db_column='deleted_by_id', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='%(class)s_deleted', to=settings.AUTH_USER_MODEL)),
                ('exams', models.ManyToManyField(blank=True, help_text='Conjunto de exames laboratoriais da bandeja deste perfil.', related_name='occupational_profiles', to='clinical.labexam', verbose_name='Exames do perfil')),
                ('tenant', models.ForeignKey(db_column='tenant_id', on_delete=django.db.models.deletion.PROTECT, related_name='%(class)ss', to='inquilinos.tenant')),
                ('updated_by', models.ForeignKey(blank=True, db_column='updated_by_id', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='%(class)s_atualizado', to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'Perfil ocupacional de exames',
                'verbose_name_plural': 'Perfis ocupacionais de exames',
                'db_table': 'clinico_perfil_ocupacional',
                'ordering': ['name'],
            },
        ),
        migrations.AddField(
            model_name='labrequest',
            name='occupational_profile',
            field=models.ForeignKey(blank=True, db_column='occupational_profile_id', help_text='Perfil ocupacional cujos exames são adicionados à requisição.', null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='lab_requests', to='clinical.occupationalexamprofile', verbose_name='Perfil profissional (bandeja de exames)'),
        ),
    ]
