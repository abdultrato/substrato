...
def get_tenant_timezone(inquilino):
    """
...
def get_local_datetime(inquilino, data_hora):
    tz = get_tenant_timezone(inquilino)
    if not data_hora:
        return None
...
def calculate_schedule_type(inquilino, data_hora, feriado_manual: bool = False) -> str:
    dt_local = get_local_datetime(inquilino, data_hora)
    if not dt_local:
...
def calcular_multiplicador_preco(inquilino, data_hora, feriado_manual: bool = False) -> Decimal:
    tipo = calculate_schedule_type(inquilino, data_hora, feriado_manual=feriado_manual)
...
obter_timezone_inquilino = get_tenant_timezone
obter_datetime_local = get_local_datetime
calcular_tipo_horario = calculate_schedule_type
