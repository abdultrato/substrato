...
function classifySample(row: SampleRow): ColumnKey {
  switch ((row.status || "").trim().toUpperCase()) {
    case "RECEBIDA": return "recebidas"
    case "ACEITE":
    case "EM_PROCESSAMENTO":
    case "PROCESSADA":
    case "ARMAZENADA":
    case "DESCARTADA": return "aceites"
    case "REJEITADA": return "rejeitadas"
    default: return "a_receber"
  }
}
...
function statusDot(status?: string) {
  switch ((status || "").trim().toUpperCase()) {
    ...
    case "PROCESSADA":
    case "ARMAZENADA":
    case "DESCARTADA": return "bg-emerald-400"
    case "RECEBIDA": return "bg-amber-400"
    case "REJEITADA": return "bg-rose-400"
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 border-t border-white/40 px-3 py-2 dark:border-white/10">
        <div className="min-w-0 text-[9px] text-[var(--gray-400)]">
          <p className="truncate">Colhida {fmt(row.collected_at)}</p>
          {row.received_at && (
            <p className="truncate">Recebida {fmt(row.received_at)}</p>
          )}
        </div>
...
export default function SamplesBoardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { items } = await apiFetchList<SampleRow>(
        `/clinical_laboratory/sample/${qs ? `?${qs}` : ""}`,
  ...
  const buckets = useMemo(() => {
    const grouped: Record<ColumnKey, SampleRow[]> = {
      a_receber: [], recebidas: [], aceites: [], rejeitadas: [],
    }
