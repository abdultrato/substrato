...
import { Building2, CalendarDays, ClipboardList, HeartPulse, User } from "lucide-react"
...
export default function NursingEvolutionDetails({
  data,
  actions,
}: {
  endpoint: string
  ...
  const evolutionDate = formatDateTime(data?.evolution_date, isPt)
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--gray-500)]">
                {t("Evolução de enfermagem", "Nursing evolution")}
              </p>
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <MetaItem icon={<CalendarDays size={15} />} label={t("Data de registo de evolução", "Evolution record date")} value={evolutionDate} />
      </div>

      {/* Cartão principal — texto da evolução */}
      <section className="rounded-xl border border-white/20 bg-white/30 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/[0.04]">
        <div className="flex items-center gap-2 border-b border-white/20 px-4 py-2.5 dark:border-white/10">
          ...
          <h3 className="text-sm font-semibold text-[var(--text)]">{t("Evolução clínica", "Clinical evolution")}</h3>
        </div>
        <div className="px-4 py-3">
          {observation ? (
            <p className="whitespace-pre-wrap text-sm leading-relaxed text-[var(--text)]">{observation}</p>
          ) : (
            <p className="text-sm italic text-[var(--gray-500)]">{t("Sem texto de evolução registado.", "No evolution text recorded.")}</p>
          )}
        </div>
      </section>
    </div>
