...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function EditImmunizationPage() {
  useAuthGuard();
  ...
  useEffect(() => {
    if (!id) return;
    apiFetch<Record<string, unknown>>(`/public_health/immunization/${id}/`)
      .then((d) => {
        setCustomId(String(d.custom_id ?? ""));
        setPatientId(d.patient as number ?? null);
        setPatientLabel(String(d.patient_name ?? (d.patient ? `Paciente #${d.patient}` : "")));
        setVaccineId(d.vaccine as number ?? null);
        setVaccineLabel(String(d.vaccine_name ?? (d.vaccine ? `Vacina #${d.vaccine}` : "")));
        setLotId(d.lot as number ?? null);
        setLotLabel(String(d.lot_number ?? (d.lot ? `Lote #${d.lot}` : "")));
        setCampaignId(d.campaign as number ?? null);
        setCampaignLabel(String(d.campaign_name ?? (d.campaign ? `Campanha #${d.campaign}` : "")));
        setEmployeeId(d.administered_by as number ?? null);
        setEmployeeLabel(String(d.administered_by_name ?? (d.administered_by ? `Funcionário #${d.administered_by}` : "")));
        setStatus(String(d.status ?? "ADMINISTERED"));
        setSource(String(d.source ?? "ROUTINE"));
        setDoseNumber(String(d.dose_number ?? 1));
        setAdministeredAt(d.administered_at ? String(d.administered_at).substring(0, 16) : "");
        setNextDueDate(String(d.next_due_date ?? ""));
        setRoute(String(d.route ?? "IM"));
        setBodySite(String(d.body_site ?? ""));
        setConsentConfirmed(Boolean(d.consent_confirmed ?? true));
        setContraindicationReason(String(d.contraindication_reason ?? ""));
        setOfficialNotificationId(String(d.official_notification_id ?? ""));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/public_health/immunization/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          patient: patientId,
          vaccine: vaccineId,
      ...
      router.push(`/public-health/immunizations/${id}`);
    } catch (err: unknown) {
          ...
          <Card icon={User} title="Paciente e profissional" accent={activeStatus.bar}>
            <Field label="Paciente" required error={errors.patient}>
              <RelationSelect target={T_PATIENT} value={patientId}
                onChange={(v, lbl) => { setPatientId(v); setPatientLabel(lbl); if (v) setErrors((p) => ({ ...p, patient: "" })); }}
                placeholder="Pesquisar paciente…" initialLabel={patientLabel || undefined} />
            </Field>
            <Field label="Aplicada por">
              <RelationSelect target={T_EMPLOYEE} value={employeeId}
                onChange={(v, lbl) => { setEmployeeId(v); setEmployeeLabel(lbl); }}
                placeholder="Pesquisar profissional…" initialLabel={employeeLabel || undefined} />
            </Field>
            <Field label="Origem" hint="Tipo de administração">
          ...
          <Card icon={Syringe} title="Vacina e lote" accent="bg-teal-500">
            <Field label="Vacina" required error={errors.vaccine}>
              <RelationSelect target={T_VACCINE} value={vaccineId}
                onChange={(v, lbl) => { setVaccineId(v); setVaccineLabel(lbl); setLotId(null); setLotLabel(""); if (v) setErrors((p) => ({ ...p, vaccine: "" })); }}
                placeholder="Pesquisar vacina…" initialLabel={vaccineLabel || undefined} />
            </Field>
            <Field label="Lote" required={status === "ADMINISTERED" || status === "REPORTED"} error={errors.lot}
              hint="Obrigatório para registos aplicados">
              <RelationSelect target={T_LOT} value={lotId}
                onChange={(v, lbl) => { setLotId(v); setLotLabel(lbl); if (v) setErrors((p) => ({ ...p, lot: "" })); }}
                placeholder="Pesquisar lote…" initialLabel={lotLabel || undefined} />
            </Field>
          ...
          <Card icon={ClipboardCheck} title="Campanha (opcional)" accent="bg-indigo-500">
            <Field label="Campanha">
              <RelationSelect target={T_CAMPAIGN} value={campaignId}
                onChange={(v, lbl) => { setCampaignId(v); setCampaignLabel(lbl); }}
