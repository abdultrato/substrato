...
export default function EditAEFIPage() {
  useAuthGuard();
  ...
  useEffect(() => {
    if (!id) return;
    apiFetch<Record<string, unknown>>(`/public_health/adverse_event/${id}/`)
      .then((d) => {
        setCustomId(String(d.custom_id ?? ""));
        setImmunizationId(d.immunization_record as number ?? null);
        setImmunizationLabel(String(d.record_label ?? (d.immunization_record ? `IMR #${d.immunization_record}` : "")));
        setReportedById(d.reported_by as number ?? null);
        setReportedByLabel(String(d.reported_by_name ?? (d.reported_by ? `Funcionário #${d.reported_by}` : "")));
        setInvestigatedById(d.investigated_by as number ?? null);
        setInvestigatedByLabel(String(d.investigated_by_name ?? (d.investigated_by ? `Funcionário #${d.investigated_by}` : "")));
        setSeverity(String(d.severity ?? "MILD"));
        setStatus(String(d.status ?? "REPORTED"));
        setOnsetAt(d.onset_at ? String(d.onset_at).substring(0, 16) : "");
        setReportedAt(d.reported_at ? String(d.reported_at).substring(0, 16) : "");
        setInvestigationDueAt(d.investigation_due_at ? String(d.investigation_due_at).substring(0, 16) : "");
        setSymptoms(String(d.symptoms ?? ""));
        setSerious(Boolean(d.serious ?? false));
        setOutcome(String(d.outcome ?? "UNKNOWN"));
        setCausalityAssessment(String(d.causality_assessment ?? ""));
        setOfficialNotificationId(String(d.official_notification_id ?? ""));
        setNotes(String(d.notes ?? ""));
      })
      .catch(() => setSaveError("Erro ao carregar evento adverso."))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/public_health/adverse_event/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
      ...
      router.push(`/public-health/adverse-events/${id}`);
    } catch (err: unknown) {
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                <span>Eventos Adversos</span><span>/</span>
                <span className="font-mono">{customId}</span><span>/</span>
                <span className="font-medium text-foreground">Editar</span>
              </div>
              <h1 className="text-base font-bold leading-tight text-foreground">Editar evento adverso</h1>
              <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                ...
                {serious && (
                  <span className="inline-flex items-center rounded-full border border-red-300 bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-800 dark:border-red-600/40 dark:bg-red-900/30 dark:text-red-300">⚠ Grave</span>
                )}
              </div>
            </div>
          ...
          <Card icon={Syringe} title="Registo e profissionais" accent={activeStatus.bar}>
            <Field label="Registo de imunização" required error={errors.immunization}>
              <RelationSelect target={T_IMMUNIZATION} value={immunizationId} zIndex="z-[30]"
                onChange={(v, lbl) => { setImmunizationId(v); setImmunizationLabel(lbl); if (v) setErrors((p) => ({ ...p, immunization: "" })); }}
                placeholder="Pesquisar registo…" initialLabel={immunizationLabel || undefined} />
            </Field>
            <Field label="Reportado por">
              <RelationSelect target={T_EMPLOYEE} value={reportedById} zIndex="z-[20]"
                onChange={(v, lbl) => { setReportedById(v); setReportedByLabel(lbl); }}
                placeholder="Pesquisar profissional…" initialLabel={reportedByLabel || undefined} />
            </Field>
            <Field label="Investigado por">
              <RelationSelect target={T_EMPLOYEE} value={investigatedById} zIndex="z-[10]"
                onChange={(v, lbl) => { setInvestigatedById(v); setInvestigatedByLabel(lbl); }}
                placeholder="Pesquisar profissional…" initialLabel={investigatedByLabel || undefined} />
            </Field>
          </Card>
