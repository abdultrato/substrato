...
const STATUSES = [
  { value: "RECEIVED",    label: "Recebido",   emoji: "📦", bar: "bg-blue-500",    chip: "border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-700/40 dark:bg-blue-900/20 dark:text-blue-300",             grad: "from-blue-500 to-indigo-600",   glow: "shadow-blue-500/30"    },
  { value: "ACTIVE",      label: "Ativo",      emoji: "✅", bar: "bg-emerald-500", chip: "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-700/40 dark:bg-emerald-900/20 dark:text-emerald-300", grad: "from-emerald-500 to-teal-600",  glow: "shadow-emerald-500/30" },
  { value: "QUARANTINED", label: "Quarentena", emoji: "🚧", bar: "bg-amber-500",   chip: "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-700/40 dark:bg-amber-900/20 dark:text-amber-300",         grad: "from-amber-500 to-orange-600",  glow: "shadow-amber-500/30"   },
  { value: "DEPLETED",    label: "Esgotado",   emoji: "🔻", bar: "bg-slate-400",   chip: "border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-700/40 dark:bg-slate-800/20 dark:text-slate-300",         grad: "from-slate-400 to-slate-500",   glow: "shadow-slate-400/30"   },
  { value: "EXPIRED",     label: "Expirado",   emoji: "⌛", bar: "bg-red-500",     chip: "border-red-200 bg-red-50 text-red-700 dark:border-red-700/40 dark:bg-red-900/20 dark:text-red-300",                 grad: "from-red-500 to-rose-600",      glow: "shadow-red-500/30"     },
  { value: "RECALLED",    label: "Recolhido",  emoji: "↩️", bar: "bg-rose-500",    chip: "border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-700/40 dark:bg-rose-900/20 dark:text-rose-300",             grad: "from-rose-500 to-red-600",      glow: "shadow-rose-500/30"    },
];
...
export default function NewLotPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (officialBatch.trim()) body.official_batch_code = officialBatch.trim();
      ...
      if (storageTemp.trim()) body.storage_temperature_c = Number(storageTemp);
      if (notes.trim()) body.notes = notes.trim();

      const res = await apiFetch<{ id: number }>("/public_health/lot/", {
        method: "POST",
        body: JSON.stringify(body),
      });
      router.push(`/public-health/lots/${res.id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao registar lote.");
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                ...
                <span className="font-medium text-foreground">Novo lote</span>
              </div>
              <h1 className="text-base font-bold leading-tight text-foreground">
                {vaccineLabel ? `Lote — ${vaccineLabel}${lotNumber ? ` · ${lotNumber}` : ""}` : "Novo lote de vacina"}
              </h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="submit" disabled={saving}
                className={`inline-flex h-8 items-center gap-1.5 rounded-lg bg-gradient-to-r px-4 text-xs font-semibold text-white shadow-md transition disabled:opacity-50 ${activeStatus.grad} ${activeStatus.glow}`}>
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={Syringe} title="Vacina e identificação" accent="bg-sky-500">
            <Field label="Vacina" required error={errors.vaccine}>
              <RelationSelect target={T_VACCINE} value={vaccineId}
                onChange={(id, lbl) => { setVaccineId(id); setVaccineLabel(lbl); if (id) setErrors((p) => ({ ...p, vaccine: "" })); }}
          ...
          <Card icon={PackageCheck} title="Doses" accent="bg-indigo-500">
            ...
            <p className="text-[10px] text-muted-foreground">
              Disponíveis ≤ recebidas · reservadas ≤ disponíveis. Se deixar as disponíveis vazias, o sistema assume as recebidas.
            </p>
          </Card>
