...
const DASHBOARD_GROUPS = [
  GROUPS.ADMIN, GROUPS.RECEPCAO, GROUPS.MEDICINA, GROUPS.MEDICINA_OCUPACIONAL,
  GROUPS.ENFERMAGEM, GROUPS.LABORATORIO, GROUPS.SAUDE_PUBLICA,
];

/* ── Types ─────────────────────────────────────────────────────────── */

interface Summary {
  vaccines: number; active_lots: number; active_campaigns: number;
  immunizations_30d: number; due_boosters: number; serious_aefi_open: number;
  pending_notifications: number; low_stock_lots: number;
  cold_chain_breaches: number; expired_lots: number;
}

interface StockRisk {
  id: number; custom_id?: string; vaccine_name?: string; lot_number?: string;
  expiration_date?: string; doses_available?: number; storage_location?: string; risk?: string;
}

interface CampaignItem {
  id: number; custom_id?: string; name?: string; vaccine_name?: string;
  target_region?: string; administered_doses?: number; target_doses?: number; coverage_percent?: string | number;
}

interface BoosterItem {
  id: number; custom_id?: string; patient_name?: string; vaccine_name?: string;
  dose_number?: number; next_due_date?: string; days_overdue?: number;
}

interface AefiItem {
  id: number; custom_id?: string; patient_name?: string; vaccine_name?: string;
  severity?: string; serious?: boolean; reported_at?: string; investigation_due_at?: string | null;
}

interface NotifItem {
  id: number; custom_id?: string; official_system?: string; event_type?: string;
  status?: string; attempt_count?: number; next_retry_at?: string | null; error_message?: string;
}
...
function fmtDate(v?: string | null): string {
  if (!v) return "—";
  const d = new Date(v);
  return Number.isNaN(d.getTime()) ? v : d.toLocaleDateString("pt-PT", { day: "2-digit", month: "short", year: "numeric" });
}
...
function parseData(raw: DashboardData) {
  ...
  const summary: Summary = {
    ...
    due_boosters:          num(s.due_boosters ?? s.overdue_boosters ?? s.reforcos_vencidos),
    serious_aefi_open:     num(s.serious_aefi_open ?? s.aefi_graves),
    pending_notifications: num(s.pending_notifications ?? s.notificacoes_pendentes),
  ...
  return {
    ...
    notifications: arr<NotifItem>(raw.notification_queue ?? raw.pending_notifications),
  };
}

/* ── Metric row ────────────────────────────────────────────────────── */

type MetricItem = { label: string; value: number; bar: string; chip: string; alert?: boolean };

function MetricPill({ m, loading }: { m: MetricItem; loading: boolean }) {
  return (
    <div className={`relative overflow-hidden rounded-xl border border-white/20 bg-white/25 px-2.5 py-2 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5 ${m.alert ? "ring-1 ring-red-400/40" : ""}`}>
      <span className={`absolute inset-y-0 left-0 w-1 rounded-l-xl ${m.bar}`} />
      <div className="pl-1">
        {loading
          ? <div className="h-4 w-8 animate-pulse rounded bg-current/20" />
          : <div className={`text-base font-bold tabular-nums leading-none ${m.chip}`}>{m.value}</div>}
        <div className="mt-0.5 text-[9px] leading-tight text-muted-foreground">{m.label}</div>
        {m.alert && <div className="mt-0.5 text-[9px] font-semibold text-red-600 dark:text-red-400">⚠ atenção</div>}
      </div>
    </div>
  );
}
...
type QueueCardProps = {
  icon: LucideIcon; title: string; count: number; href: string;
  bar: string; iconBg: string; badge: string; loading: boolean;
  children: React.ReactNode;
};

function QueueCard({ icon: Icon, title, count, href, bar, iconBg, badge, loading, children }: QueueCardProps) {
  return (
      ...
      <div className="flex items-center gap-2.5 border-b border-border/50 px-3 py-2 pl-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            {!loading && (
              <span className={`rounded-full border px-1.5 py-0.5 text-[9px] font-bold tabular-nums ${badge}`}>{count}</span>
            )}
          </div>
        </div>
        <Link href={href} className="inline-flex shrink-0 items-center gap-1 text-[11px] font-semibold text-primary hover:underline">
      ...
      <div className="space-y-1 p-2 pl-3">
        {loading ? (
          <div className="space-y-1">
            {[0,1,2].map((i) => <div key={i} className="h-10 animate-pulse rounded-lg bg-white/35 dark:bg-white/10" />)}
          </div>
        ) : children}
      </div>
    </section>
...
function QueueRow({ href, title, subtitle, pill, pillClass }: {
  href: string; title: string; subtitle: string; pill?: string; pillClass?: string;
}) {
  return (
    <Link href={href} className="flex items-center justify-between gap-2 rounded-lg border border-white/20 bg-white/25 px-2.5 py-2 transition hover:bg-white/40 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10">
      ...
      {pill && <span className={`shrink-0 rounded-full border px-1.5 py-0.5 text-[9px] font-semibold ${pillClass}`}>{pill}</span>}
...
export default function PublicHealthDashboardPage() {
  ...
  const s: Summary = data?.summary ?? {
    vaccines: 0, active_lots: 0, active_campaigns: 0, immunizations_30d: 0,
    due_boosters: 0, serious_aefi_open: 0, pending_notifications: 0,
    low_stock_lots: 0, cold_chain_breaches: 0, expired_lots: 0,
  };

  const metrics: MetricItem[] = [
    { label: "Vacinas",           value: s.vaccines,           bar: "bg-emerald-500", chip: "text-emerald-700 dark:text-emerald-300" },
    { label: "Lotes ativos",      value: s.active_lots,        bar: "bg-amber-500",   chip: "text-amber-700   dark:text-amber-300"   },
    { label: "Campanhas ativas",  value: s.active_campaigns,   bar: "bg-blue-500",    chip: "text-blue-700    dark:text-blue-300"    },
    { label: "Imunizações (30d)", value: s.immunizations_30d,  bar: "bg-teal-500",    chip: "text-teal-700    dark:text-teal-300"    },
    { label: "Reforços vencidos", value: s.due_boosters,       bar: "bg-orange-500",  chip: "text-orange-700  dark:text-orange-300",  alert: s.due_boosters > 0 },
    { label: "AEFI graves",       value: s.serious_aefi_open,  bar: "bg-red-500",     chip: "text-red-700     dark:text-red-300",     alert: s.serious_aefi_open > 0 },
    { label: "Notif. pendentes",  value: s.pending_notifications, bar: "bg-violet-500", chip: "text-violet-700 dark:text-violet-300", alert: s.pending_notifications > 0 },
    { label: "Riscos de stock",   value: s.low_stock_lots + s.cold_chain_breaches + s.expired_lots, bar: "bg-rose-500", chip: "text-rose-700 dark:text-rose-300", alert: (s.low_stock_lots + s.cold_chain_breaches + s.expired_lots) > 0 },
  ];
    ...
    <AppLayout requiredGroups={DASHBOARD_GROUPS}>
      <div className="mx-auto w-[90%] space-y-2">
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                <Link href="/public-health" className="hover:underline">Saúde Pública</Link>
                <span>/</span>
                <span className="font-medium text-foreground">Dashboard operacional</span>
              </div>
              <h1 className="text-base font-bold leading-tight text-foreground">Monitorização em tempo real</h1>
              <div className="mt-0.5 text-[10px] text-muted-foreground">
                Filas operacionais · stock · AEFI · notificações pendentes
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => setRefreshed((n) => n + 1)} disabled={loading}
        ...
        <div className="grid grid-cols-4 gap-1.5 xl:grid-cols-8">
          {metrics.map((m) => <MetricPill key={m.label} m={m} loading={loading} />)}
        </div>
        ...
        <div className="grid gap-2 xl:grid-cols-2">
          ...
          {/* Lotes em risco */}
          <QueueCard icon={PackageCheck} title="Lotes em risco" count={data?.stockRisks.length ?? 0}
            href="/public-health/lots" loading={loading}
            ...
            {data?.stockRisks.length ? data.stockRisks.map((item) => (
          ...
          {/* Campanhas ativas */}
          <QueueCard icon={ClipboardList} title="Campanhas ativas" count={data?.campaigns.length ?? 0}
            href="/public-health/campaigns" loading={loading}
            ...
            {data?.campaigns.length ? data.campaigns.map((item) => (
              <QueueRow key={item.id}
                ...
                pillClass={Number(item.coverage_percent ?? 0) >= 80
          ...
          {/* Reforços vencidos */}
          <QueueCard icon={Syringe} title="Reforços vencidos" count={data?.boosters.length ?? 0}
            href="/public-health/immunizations" loading={loading}
            ...
            {data?.boosters.length ? data.boosters.map((item) => (
              <QueueRow key={item.id}
          ...
          <QueueCard icon={AlertTriangle} title="AEFI em investigação" count={data?.aefi.length ?? 0}
            href="/public-health/adverse-events" loading={loading}
            ...
            {data?.aefi.length ? data.aefi.map((item) => (
              <QueueRow key={item.id}
          ...
          <div className="xl:col-span-2">
            <QueueCard icon={Bell} title="Notificações oficiais pendentes" count={data?.notifications.length ?? 0}
              href="/public-health/notifications" loading={loading}
        ...
        <div className="flex items-center justify-between rounded-xl border border-white/20 bg-white/20 px-4 py-2 text-[11px] text-muted-foreground backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
          ...
          <div className="flex items-center gap-3">
            <Link href="/public-health" className="flex items-center gap-1 text-primary hover:underline">
              <ArrowLeft size={11} /> Hub de Saúde Pública
            </Link>
            <Link href="/admin/saude_publica/" className="flex items-center gap-1 text-primary hover:underline">
