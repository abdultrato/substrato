...
const CREATE_GROUPS = [GROUPS.ADMIN, GROUPS.SAUDE_PUBLICA];
...
export default function NewNotificationPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await apiFetch<{ id: number }>("/public_health/notification/", {
        method: "POST",
        body: JSON.stringify({
      ...
      router.push(`/public-health/notifications/${res.id}`);
    } catch (err: unknown) {
      setSaveError((err as { message?: string })?.message || "Erro ao criar notificação.");
  ...
  return (
    <AppLayout requiredGroups={CREATE_GROUPS}>
      <form onSubmit={handleSubmit} noValidate className="mx-auto w-[90%] space-y-2">
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/20 bg-white/30 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[10px] text-muted-foreground">Saúde Pública / Notificações</div>
              <h1 className="text-base font-bold leading-tight text-foreground">Nova notificação oficial</h1>
        ...
        {/* Estado — full width */}
        <Card icon={Bell} title="Estado inicial" accent={activeStatus.bar}>
          ...
          <Card icon={ClipboardList} title="Entidade notificada" accent={activeStatus.bar}>
            ...
            <Field label="Campanha" hint={eventType === "CAMPAIGN_COVERAGE" ? "Obrigatório para este tipo" : undefined}>
              <RelationSelect target={T_CAMPAIGN} value={campaignId} zIndex="z-[30]"
                onChange={(v) => { setCampaignId(v); setErrors((p) => ({ ...p, link: "" })); }}
            ...
            <Field label="Registo de imunização" hint={eventType === "IMMUNIZATION" ? "Obrigatório para este tipo" : undefined}>
              <RelationSelect target={T_IMMUNIZATION} value={immunizationId} zIndex="z-[20]"
                onChange={(v) => { setImmunizationId(v); setErrors((p) => ({ ...p, link: "" })); }}
                placeholder="Pesquisar imunização…" />
            </Field>
            <Field label="Evento adverso" hint={eventType === "AEFI" ? "Obrigatório para este tipo" : undefined}>
              <RelationSelect target={T_ADVERSE_EVENT} value={adverseEventId} zIndex="z-[10]"
                onChange={(v) => { setAdverseEventId(v); setErrors((p) => ({ ...p, link: "" })); }}
                placeholder="Pesquisar evento adverso…" />
            </Field>
          </Card>

          {/* Referência e observações — full width */}
          <div className="lg:col-span-2">
            <Card icon={CalendarDays} title="Referência e observações" accent="bg-slate-400">
