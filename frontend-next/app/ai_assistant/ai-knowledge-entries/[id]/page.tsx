...
export default function AiKnowledgeEntriesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/ai_assistant/ai-knowledge-entries/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["ai_assistant", "ai-knowledge-entries", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar AiKnowledgeEntries</div>;
  if (!item) return <div>AiKnowledgeEntries não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AiKnowledgeEntries #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar AiKnowledgeEntries"
            onSuccess={() => {
              setIsEditing(false);
