...
export default function AiOperationalTasksDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["ai_assistant", "ai-operational-tasks", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AiOperationalTasks #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar AiOperationalTasks"
            onSuccess={() => {
              setIsEditing(false);
