...
export default function AiSuggestedActionsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["ai_assistant", "ai-suggested-actions", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AiSuggestedActions #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar AiSuggestedActions"
            onSuccess={() => {
              setIsEditing(false);
