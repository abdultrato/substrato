...
export default function AiPolicyEventsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/ai_assistant/ai-policy-events/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["ai_assistant", "ai-policy-events", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar AiPolicyEvents</div>;
  if (!item) return <div>AiPolicyEvents não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AiPolicyEvents #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar AiPolicyEvents"
            onSuccess={() => {
              setIsEditing(false);
