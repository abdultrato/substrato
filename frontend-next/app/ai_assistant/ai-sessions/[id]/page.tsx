...
export default function AiSessionsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/ai_assistant/ai-sessions/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["ai_assistant", "ai-sessions", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar AiSessions</div>;
  if (!item) return <div>AiSessions não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AiSessions #${id}`}</h1>
