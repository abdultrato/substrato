...
export default function OvertimesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/human_resources/overtimes/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["human_resources", "overtimes", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar Overtimes</div>;
  if (!item) return <div>Overtimes não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `Overtimes #${id}`}</h1>
