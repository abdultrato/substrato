...
export default function DisciplinaryProcessesDetailPage() {
  ...
  const endpointBase = "/human_resources/disciplinary-processes/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["human_resources", "disciplinary-processes", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar DisciplinaryProcesses</div>;
  if (!item) return <div>DisciplinaryProcesses não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `DisciplinaryProcesses #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar DisciplinaryProcesses"
            onSuccess={() => {
              setIsEditing(false);
