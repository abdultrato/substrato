...
export default function FamilyDependentsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/human_resources/family-dependents/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["human_resources", "family-dependents", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar FamilyDependents</div>;
  if (!item) return <div>FamilyDependents não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `FamilyDependents #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar FamilyDependents"
            onSuccess={() => {
              setIsEditing(false);
