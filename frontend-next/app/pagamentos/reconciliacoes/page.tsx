...
export default function PagamentosReconciliacoesPage() {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                if (!mounted) return
                setErro(e?.message || "Falha ao carregar reconciliações.")
            } finally {
    ...
    const columns = useMemo(
        () => [
            ...
            { header: "Transação", render: (r: ReconcRow) => r.transacao || "-" },
            { header: "Confirmado", render: (r: ReconcRow) => (r.confirmado ? "Sim" : "Não") },
            { header: "Confirmado em", render: (r: ReconcRow) => fmtDateTime(r.data_confirmacao) },
            { header: "Criado em", render: (r: ReconcRow) => fmtDateTime(r.criado_em) },
        ],
        []
    )
                ...
                <PageHeader
                    ...
                    actions={
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Link
                                href="/recursos/pagamentos/reconciliacao"
