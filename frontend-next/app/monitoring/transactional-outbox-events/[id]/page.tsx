...
export default function TransactionalOutboxEventsDetailPage() {
  ...
  const endpointBase = "/monitoring/transactional-outbox-events/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["monitoring", "transactional-outbox-events", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar TransactionalOutboxEvents</div>;
  if (!item) return <div>TransactionalOutboxEvents não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `TransactionalOutboxEvents #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar TransactionalOutboxEvents"
            onSuccess={() => {
              setIsEditing(false);
