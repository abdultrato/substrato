...
export default function MonitoringErrorsPage() {
  ...
  useEffect(() => {
    ...
    async function loadList() {
      try {
        ...
        const { items, meta } = await apiFetchList<ErrorRow>(
          "/monitoring/error/",
          {
            page,
            pageSize,
    ...
    async function loadTelemetry() {
      try {
        ...
        if (!isNotFoundLikeError(error)) {
          setErrorMessage(
            (prev) => prev || error?.message || "Falha ao carregar telemetria.",
          );
        }
      } finally {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: "Estado",
        render: (row: ErrorRow) => pick(row, ["status_code"], "-"),
      },
      {
        header: "Método",
        render: (row: ErrorRow) => pick(row, ["method"], "-"),
      },
      {
        header: "Rota",
        render: (row: ErrorRow) =>
          truncate(pick(row, ["path", "caminho"], "-"), 44),
      },
      {
        header: "Tipo",
        render: (row: ErrorRow) =>
          truncate(pick(row, ["exception_class"], "-"), 28),
      },
      {
        header: "Mensagem",
        render: (row: ErrorRow) =>
          truncate(pick(row, ["message", "mensagem"], "-"), 44),
      },
      {
        header: "Utilizador",
        render: (row: ErrorRow) =>
          truncate(pick(row, ["user_name", "user", "usuario_nome"], "-"), 24),
      },
      {
        header: "Criado em",
        render: (row: ErrorRow) =>
          fmtDateTime(pick(row, ["created_at", "criado_em"], null)),
      },
    ],
    [],
  );
      ...
      <div className="space-y-6">
        ...
        {telemetry?.range ? (
          <div className="text-xs text-[var(--muted-foreground)]">
            Intervalo: {fmtDateTime(telemetry.range.start)} até{" "}
            {fmtDateTime(telemetry.range.end)}
          </div>
        ) : null}
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            ...
            value={loadingTelemetry ? "..." : (totals.errors_total ?? "—")}
          />
          <MetricCard
            label="Erros de frontend"
            value={loadingTelemetry ? "..." : (totals.frontend_errors ?? "—")}
          />
          <MetricCard
            label="Erros de backend"
            value={loadingTelemetry ? "..." : (totals.backend_errors ?? "—")}
          />
          <MetricCard
            label="Rotas afetadas"
            value={loadingTelemetry ? "..." : (totals.unique_paths ?? "—")}
          />
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            label="HTTP 4xx"
            value={loadingTelemetry ? "..." : (totals.client_4xx ?? "—")}
          />
          <MetricCard
            label="HTTP 5xx"
            value={loadingTelemetry ? "..." : (totals.server_5xx ?? "—")}
          />
          <MetricCard
            ...
            value={
              loadingTelemetry
                ? "..."
                : (totals.unique_exception_classes ?? "—")
            }
          />
          <MetricCard
            label="Utilizadores impactados"
            value={loadingTelemetry ? "..." : (totals.unique_users ?? "—")}
          />
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <Card title="Eventos monitorados da outbox">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <MetricCard
                label="Pendentes"
                value={telemetry?.outbox?.pending ?? 0}
              />
              <MetricCard
                label="Entregues"
                value={telemetry?.outbox?.delivered ?? 0}
              />
              <MetricCard
                label="Falhados / dead-letter"
                value={
                  (telemetry?.outbox?.failed ?? 0) +
                  (telemetry?.outbox?.dead_letter ?? 0)
                }
              />
            </div>
          </Card>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm text-slate-600">
            Total de eventos de erro: {totalItems}
          </div>
