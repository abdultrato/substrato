...
export default function MonitoringCommandCenterPage() {
      ...
      try {
        ...
        setErrorMessage(
          isNotFoundLikeError(error)
            ...
            : error?.message ||
                t(
                  "Falha ao carregar o Centro de comando.",
  ...
  const alertColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (row: CommandCenterAlert) =>
          (isPortuguese ? row.title_pt : row.title_en) || "-",
      },
      {
        header: t("Descrição", "Description"),
        render: (row: CommandCenterAlert) =>
          (isPortuguese ? row.description_pt : row.description_en) || "-",
      },
      {
        header: t("Valor", "Value"),
        render: (row: CommandCenterAlert) =>
          toNumber(row.value).toLocaleString(),
        className: "text-right",
      },
      {
        header: t("Meta", "Target"),
        render: (row: CommandCenterAlert) =>
          toNumber(row.target).toLocaleString(),
        className: "text-right",
      },
    ],
    [isPortuguese, t],
    ...
    () => [
      {
        ...
        render: (row: ModuleHealthRow) =>
          (isPortuguese ? row.label_pt : row.label_en) || row.module_key || "-",
      },
      {
      ...
      },
      {
        ...
        render: (row: ModuleHealthRow) =>
          toNumber(row.total_requests).toLocaleString(),
        className: "text-right",
      },
      {
        header: t("Erros 4xx/5xx", "4xx/5xx errors"),
        render: (row: ModuleHealthRow) =>
          toNumber(row.error_4xx_5xx).toLocaleString(),
        className: "text-right",
      },
      {
        header: t("Latência média (ms)", "Average latency (ms)"),
        render: (row: ModuleHealthRow) =>
          toNumber(row.avg_duration_ms).toLocaleString(),
        className: "text-right",
      },
    ],
    [isPortuguese, t],
  ...
  const routeColumns = useMemo(
    () => [
      {
        header: t("Rota", "Route"),
        render: (row: RouteHealthRow) => row.path || "-",
      },
      {
        header: t("Falhas 5xx", "5xx failures"),
        render: (row: RouteHealthRow) =>
          toNumber(row.server_5xx).toLocaleString(),
        className: "text-right",
      },
    ],
    [t],
    ...
    () => [
      {
        ...
        render: (row: ScheduledReportRow) =>
          (isPortuguese ? row.label_pt : row.label_en) || row.key || "-",
      },
      {
      ...
      <div className="space-y-6">
        ...
        {data?.range ? (
          <div className="text-xs text-[var(--muted-foreground)]">
            {t("Intervalo", "Range")}: {fmtDateTime(data.range.start)}{" "}
            {t("até", "to")} {fmtDateTime(data.range.end)}
          </div>
        ) : null}
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            label={t("Total de pedidos", "Total requests")}
            value={
              loading ? "..." : toNumber(totals.total_requests).toLocaleString()
            }
          />
          <MetricCard
            label={t("Taxa global de sucesso", "Global success rate")}
            value={loading ? "..." : fmtPercent(totals.success_rate)}
          />
          <MetricCard
            label={t("Alertas ativos", "Active alerts")}
            value={loading ? "..." : alerts.length}
          />
          <MetricCard
            label={t("Módulos abaixo do SLO", "Modules below SLO")}
            value={
              loading
                ? "..."
                : toNumber(totals.modules_below_slo).toLocaleString()
            }
          />
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            label={t("Erros 4xx", "4xx errors")}
            value={
              loading ? "..." : toNumber(totals.client_4xx).toLocaleString()
            }
          />
          <MetricCard
            label={t("Erros 5xx", "5xx errors")}
            value={
              loading ? "..." : toNumber(totals.server_5xx).toLocaleString()
            }
          />
          <MetricCard
            label={t("Latência média (ms)", "Average latency (ms)")}
            value={
              loading
                ? "..."
                : toNumber(totals.avg_duration_ms).toLocaleString()
            }
          />
          <MetricCard
        ...
        >
          <DataTable
            ...
            data={modules}
            emptyMessage={t("Sem dados de módulos.", "No module data.")}
          />
        </Card>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          >
            <DataTable
              ...
              emptyMessage={t(
                "Sem rotas críticas no período.",
                "No critical routes in period.",
              )}
            />
          </Card>
