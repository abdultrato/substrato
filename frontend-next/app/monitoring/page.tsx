    ...
    async function load(showSpinner = false) {
      ...
      const telemetryTotals =
        telemetryResult.status === "fulfilled"
          ? telemetryResult.value?.totals || {}
          : {};
      const activityTotals =
        telemetryResult.status === "fulfilled"
          ? telemetryResult.value?.activity_totals || {}
          : {};
      const fallbackTotal =
        errorsResult.status === "fulfilled"
          ? extractTotalCount(errorsResult.value)
          : 0;
      const telemetryTotalRaw = telemetryTotals.errors_total;
      const hasTelemetryTotal =
        telemetryTotalRaw !== undefined && telemetryTotalRaw !== null;
      const telemetryAvailable = telemetryResult.status === "fulfilled";

      const combinedTotal = Math.max(
        toNumber(telemetryTotals.errors_total),
        toNumber(activityTotals.errors_total),
      );
      const combined4xx = Math.max(
        toNumber(telemetryTotals.client_4xx),
        toNumber(activityTotals.client_4xx),
      );
      const combined5xx = Math.max(
        toNumber(telemetryTotals.server_5xx),
        toNumber(activityTotals.server_5xx),
      );
      const combinedPaths = Math.max(
        toNumber(telemetryTotals.unique_paths),
        toNumber(activityTotals.unique_paths),
      );
      ...
      if (allFailed) {
        const message =
          ...
          listError?.message ||
          "Falha ao carregar monitorização.";
        setErro(
          isNotFoundLikeError(telemetryError) || isNotFoundLikeError(listError)
            ? null
            : message,
        );
      } else {
  ...
  return (
    <AppLayout requiredGroups={[GROUPS.ADMIN]}>
      <div className="space-y-6">
        <PageHeader title="Monitorização" />
        ...
        <Card title="Sobre esta página">
          <div className="text-sm text-foreground-2">
            As métricas combinam erros monitorados e atividade HTTP dos últimos{" "}
            {REPORT_WINDOW_DAYS} dias, com atualização automática a cada{" "}
            {AUTO_REFRESH_MS / 1000} segundos.
          </div>
        </Card>
      </div>
    </AppLayout>
