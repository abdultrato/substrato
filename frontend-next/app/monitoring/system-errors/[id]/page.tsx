...
export default function SystemErrorsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/monitoring/system-errors/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["monitoring", "system-errors", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar SystemErrors</div>;
  if (!item) return <div>SystemErrors não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `SystemErrors #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar SystemErrors"
            onSuccess={() => {
              setIsEditing(false);
