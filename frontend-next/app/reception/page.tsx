...
const atalhos = [
  ...
  },
  {
    ...
    description:
      "Abrir o formulário para solicitar consumíveis ao almoxarifado/farmácia.",
  ...
  },
  {
    ...
    description:
      "Marcar procedimento de enfermagem para um paciente (catálogo, materiais e execução).",
...
const marcacoesPorSector = [
  ...
  },
  {
    ...
    description:
      "Agendar cirurgia pequena, grande ou geral com procedimento e data prevista.",
  ...
  },
  {
    title: "Odontologia",
    description:
      "Marcar consulta dentária com cadeira/gabinete, motivo e estado do atendimento.",
  ...
  },
  {
    ...
    description:
      "Marcar consulta veterinária para animal/paciente e tutor responsável.",
  ...
  },
  {
    ...
    description:
      "Abrir marcação de avaliação funcional inicial antes do plano e das sessões.",
...
export default function RecepcaoPage() {
  ...
  const [workspace, setWorkspace] =
    useState<ReceptionWorkspace>(EMPTY_WORKSPACE);
    ...
    async function carregarNotasCredito() {
      try {
        const [apro, reje] = await Promise.all([
          apiFetch<any>(
            "/billing/credit-note-request/?status=APRO&ordering=-reviewed_at",
            { clientCache: false },
          ),
          apiFetch<any>(
            "/billing/credit-note-request/?status=REJE&ordering=-reviewed_at",
            { clientCache: false },
          ),
        ]);
        setApprovedNotes(
          Array.isArray(apro?.results ?? apro) ? (apro?.results ?? apro) : [],
        );
        setRejectedNotes(
          Array.isArray(reje?.results ?? reje) ? (reje?.results ?? reje) : [],
        );
      } catch {
        // silently ignore
      } finally {
      ...
      <AppLayout requiredGroups={[GROUPS.ADMIN, GROUPS.RECEPCAO]}>
        <div className="space-y-3">
          ...
          <div className="relative overflow-hidden rounded-xl border border-white/20 bg-white/30 px-4 py-2.5 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
            ...
            <div
              aria-hidden
              className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-[var(--primary-200)] opacity-20 blur-3xl dark:bg-[var(--primary-700)]"
            />
          </div>
            ...
            <section className="rounded-xl border border-white/20 bg-white/25 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
              ...
              <div className="flex flex-col gap-1.5 p-2">
                ...
                >
                  ...
                  <ChevronRight
                    size={12}
                    className="shrink-0 text-primary-foreground/70 opacity-0 transition-opacity group-hover:opacity-100"
                  />
                </button>
                {atalhos.map((atalho) => (
                  <Link
              ...
              <section className="rounded-xl border border-white/20 bg-white/25 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
                ...
                <div className="flex flex-col gap-1 p-2">
                  {marcacoesPorSector.map((sector) => (
                    ...
                    >
                      <span
                        className={`inline-flex h-6 w-6 shrink-0 items-center justify-center rounded ${sector.iconBg}`}
                      >
...
function ReceptionDecidedSection({
  ...
  return (
    <section className="rounded-xl border border-white/20 bg-white/25 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
      <div className="flex items-center gap-2 border-b border-border/60 px-3 py-2">
        <div className="inline-flex h-6 w-6 items-center justify-center rounded bg-muted">
          {icon}
        </div>
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-1">
                  <div className="min-w-0">
                    ...
                    {r.invoice_code && (
                      <span className={`ml-1.5 text-[11px] ${subTextColor}`}>
                        {r.invoice_code}
                      </span>
                    )}
                    {r.patient_name && (
                  ...
                  <span
                    className={`rounded-md px-2 py-0.5 text-[11px] font-semibold ${amountBg}`}
                  >
                    <MoneyValue value={r.amount} />
                  </span>
                </div>
                {r.reason && (
                  <p className={`mt-1 text-[11px] ${subTextColor}`}>
                    {r.reason}
                  </p>
                )}
                <div
                  className={`mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] ${subTextColor}`}
                >
                  {r.reviewed_by_name && (
                    <span>
                ...
                {r.decision_note && (
                  <p className={`mt-1 text-[11px] italic ${subTextColor}`}>
                    &quot;{r.decision_note}&quot;
                  </p>
                )}
              </div>
...
function normalizeReceptionWorkspace(raw: any): ReceptionWorkspace {
  ...
  return {
    ...
    summary: {
      checkins_today: Number(
        summary?.checkins_today ?? summary?.checkins_hoje ?? 0,
      ),
      queue_size: Number(summary?.queue_size ?? summary?.na_fila ?? 0),
      in_care: Number(summary?.in_care ?? summary?.em_atendimento ?? 0),
      new_patients: Number(
        summary?.new_patients ?? summary?.pacientes_novos ?? 0,
      ),
      pending_requests: Number(
        summary?.pending_requests ?? summary?.requisicoes_pendentes ?? 0,
      ),
      open_invoices: Number(
        summary?.open_invoices ?? summary?.faturas_em_aberto ?? 0,
      ),
      receipts_generated_today: Number(
        summary?.receipts_generated_today ?? summary?.recibos_gerados_hoje ?? 0,
      ),
      received_today: String(
        summary?.received_today ?? summary?.recebido_hoje ?? "0.00",
      ),
    },
    queue: Array.isArray(queue)
      ? queue.map((item: any) => ({
          ...
          patient_code: String(
            item?.patient_code ?? item?.paciente_codigo ?? "",
          ),
          ...
          request_code: String(
            item?.request_code ?? item?.requisicao_codigo ?? "",
          ),
...
function KpiCard({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-xl border border-l-4 border-white/20 bg-white/30 px-3 py-2.5 shadow-sm backdrop-blur-sm dark:bg-white/5 ${accentClass}`}
    >
      <span
        className={`absolute right-2.5 top-2.5 inline-flex h-7 w-7 items-center justify-center rounded-lg ${iconBg}`}
      >
...
function IndicadorTile({
  ...
  const inner = (
    <div
      className={`flex flex-col gap-0.5 rounded-lg border border-l-4 border-white/20 bg-white/25 px-2.5 py-2 backdrop-blur-sm dark:bg-white/5 ${bar ?? "border-l-border"}`}
    >
      <span className="text-[10px] font-medium text-muted-foreground leading-none">
        {label}
      </span>
      <span
        className={`text-lg font-bold leading-tight ${accent ?? "text-foreground"}`}
      >
        {value}
      </span>
    </div>
