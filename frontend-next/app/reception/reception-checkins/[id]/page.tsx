...
export default function ReceptionCheckinsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/reception/reception-checkins/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["reception", "reception-checkins", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ReceptionCheckins</div>;
  if (!item) return <div>ReceptionCheckins não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ReceptionCheckins #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ReceptionCheckins"
            onSuccess={() => {
              setIsEditing(false);
