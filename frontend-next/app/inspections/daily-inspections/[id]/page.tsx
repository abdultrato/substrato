...
export default function DailyInspectionsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["inspections", "daily-inspections", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `DailyInspections #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar DailyInspections"
            onSuccess={() => {
              setIsEditing(false);
