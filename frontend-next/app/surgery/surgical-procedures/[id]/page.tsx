...
export default function SurgicalProceduresDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["surgery", "surgical-procedures", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `SurgicalProcedures #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar SurgicalProcedures"
            onSuccess={() => {
              setIsEditing(false);
