    ...
    <AppLayout requiredGroups={[GROUPS.ADMIN, GROUPS.MEDICINA, GROUPS.ENFERMAGEM]}>
      <div className="mx-auto w-full max-w-5xl space-y-3 px-1">
        ...
        <section className={`relative overflow-hidden ${GLASS}`}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 pl-5">
            ...
            <div className="flex items-center gap-2">
              {/* search */}
              <div className="flex h-7 items-center gap-1.5 rounded-lg border border-border bg-card px-2.5">
        ...
        {loading ? (
          ...
          <div className={`flex flex-col items-center gap-2 py-12 ${GLASS}`}>
            ...
            {filtered.map(p => {
              ...
              return (
                <Link key={p.id} href={`/surgery/surgical-procedures/${p.id}`}
                  className={`relative flex flex-col gap-2 overflow-hidden rounded-xl border border-white/30 bg-white/30 p-3 shadow-sm backdrop-blur-sm transition hover:shadow-md hover:border-violet-200/60 dark:border-white/10 dark:bg-white/[0.04]`}>
                  {/* active bar */}
                  ...
                  <div className="pl-2">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="flex-1 text-[13px] font-semibold leading-snug text-foreground">{p.name}</h3>
                    ...
                    {/* description */}
                    {p.description && (
                      <p className="mt-1 line-clamp-2 text-[11px] text-[var(--gray-500)]">{p.description}</p>
                    )}

                    {/* price row */}
                    <div className="mt-2.5 flex items-end justify-between border-t border-white/20 pt-2 dark:border-white/10">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] font-medium uppercase tracking-wide text-[var(--gray-400)]">Preço base</span>
                        <span className="text-[12px] font-semibold text-foreground">{fmtMT(p.base_price)}</span>
                      </div>
                      {p.applies_vat_by_default && vat > 0 && (
                        <div className="flex flex-col items-end gap-0.5">
                          ...
                          <span className="text-[13px] font-bold text-teal-600 dark:text-teal-400">{fmtMT(String(total))}</span>
                        </div>
                      )}
                    </div>
                  </div>
