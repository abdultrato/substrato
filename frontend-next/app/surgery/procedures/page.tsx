...
export default function SurgicalProceduresPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const { items, meta } = await apiFetchList<ProcedureRow>(
        ...
        setErrorMessage(
          isNotFoundLikeError(e)
            ? null
            : e?.message || "Falha ao carregar procedimentos.",
        );
      } finally {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: "Nome",
        render: (p: ProcedureRow) => p.nome || p.descricao || "-",
      },
      {
        header: "Ativo",
        render: (p: ProcedureRow) => (p.ativo === false ? "Não" : "Sim"),
      },
    ],
    [],
  );
