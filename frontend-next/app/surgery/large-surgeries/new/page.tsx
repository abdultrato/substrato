...
const inputCls = "w-full rounded-lg border border-white/30 bg-white/40 px-2.5 py-1.5 text-[12px] text-[var(--text)] placeholder-[var(--gray-400)] backdrop-blur-sm focus:border-indigo-400 focus:outline-none dark:border-white/10 dark:bg-white/[0.06]"
...
function DropdownPortal({ anchorRef, portalRef, open, children }: {
  ...
  return createPortal(
    <div ref={portalRef} style={{ position: "fixed", top: pos.top, left: pos.left, width: pos.width, zIndex: 9999 }}
      className="rounded-lg border border-indigo-200 bg-white shadow-xl backdrop-blur-md dark:border-white/20 dark:bg-zinc-900/90">
      {children}
    </div>,
    document.body
  )
...
function SearchSelect({ label, placeholder, endpoint, labelField = "name", value, onChange, required }: {
  ...
  return (
    <FieldRow label={label} required={required}>
      <div ref={ref}>
        <div className={`${inputCls} flex cursor-pointer items-center justify-between gap-1 ${value ? "border-indigo-300/60 dark:border-indigo-700/40" : ""}`}
          onClick={() => setOpen(o => !o)}>
          {value && selectedLabel
        ...
        <DropdownPortal anchorRef={ref} portalRef={portalRef} open={open}>
          <div className="border-b border-indigo-100 px-2 py-1.5 dark:border-white/10">
          ...
          <div className="max-h-56 overflow-y-auto">
            {results.length === 0
              ...
              : results.map(item => (
                <div key={item.id}
                  className={`cursor-pointer px-3 py-2 text-[12px] hover:bg-indigo-50 dark:hover:bg-white/10 ${item.id === value ? "bg-indigo-50 font-semibold text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-300" : "text-[var(--text)]"}`}
...
function ProcedureMultiSelect({ selected, onChange }: {
  ...
  const search = useCallback(async (q: string) => {
    try {
      const params = new URLSearchParams({ limit: "30", active: "true", for_surgery_size: "GRANDE" })
      ...
      <div ref={ref}>
        {selected.length > 0 && (
          <div className="mb-2 flex flex-wrap gap-1.5">
            {selected.map(s => (
              <span key={s.id} className="inline-flex items-center gap-1 rounded-md border border-indigo-300/60 bg-indigo-50/80 px-2 py-0.5 text-[11px] font-medium text-indigo-700 dark:border-indigo-700/40 dark:bg-indigo-900/20 dark:text-indigo-300">
                {s.name}
          ...
          <div className="max-h-56 overflow-y-auto">
            {results.length === 0
              ...
              : results.map(item => {
                ...
                return (
                  <div key={item.id}
                    className={`flex cursor-pointer items-center gap-2 px-3 py-2 text-[12px] hover:bg-indigo-50 dark:hover:bg-white/10 ${sel ? "bg-indigo-50/60 dark:bg-indigo-900/10" : ""}`}
                    onMouseDown={e => { e.preventDefault(); toggle(item) }}>
                    <span className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border ${sel ? "border-indigo-500 bg-indigo-500 text-white" : "border-[var(--gray-300)]"}`}>
                      {sel ? <Check size={9} /> : null}
                    </span>
                    <span className={sel ? "font-semibold text-indigo-700 dark:text-indigo-300" : "text-[var(--text)]"}>{item.name}</span>
                    {item.surgery_type ? (
                      <span className="rounded-full border border-indigo-200/70 bg-indigo-50 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-indigo-700 dark:border-indigo-700/40 dark:bg-indigo-900/20 dark:text-indigo-300">
          ...
          <div className="max-h-56 overflow-y-auto">
            {results.filter(r => !selected.find(s => s.id === r.id)).length === 0
              ...
              : results.filter(r => !selected.find(s => s.id === r.id)).map(item => (
                <div key={item.id}
                  className="flex cursor-pointer items-center gap-2 px-3 py-2 text-[12px] hover:bg-indigo-50 dark:hover:bg-white/10"
                  onMouseDown={e => { e.preventDefault(); toggle(item) }}>
...
export default function LargeSurgeryNewPage() {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      const d = await apiFetch<any>("/surgery/large_surgery/", {
        method: "POST",
        body: JSON.stringify({
          patient,
      ...
      router.push(`/surgery/large-surgeries/${d.id}`)
    } catch (e: any) {
      setError(e?.message || "Erro ao criar cirurgia.")
      setSaving(false)
    }
  }, [patient, procedures, surgeons, specialty, operatingRoom, preDiag, posDiag, priority, scheduledFor, router])
  ...
  return (
    <AppLayout requiredGroups={[GROUPS.ADMIN, GROUPS.ENFERMAGEM, GROUPS.MEDICINA]}>
      <div className="mx-auto w-full max-w-2xl space-y-2 px-1 py-1">
        ...
        <section className={`relative overflow-hidden ${GLASS}`}>
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-3 pl-5">
            <div>
              ...
              <h1 className="mt-0.5 font-display text-base font-semibold text-foreground">Nova grande cirurgia</h1>
            </div>
            <div className="flex items-center gap-3">
              {totalPrice > 0 && (
                <div className="flex flex-col items-end gap-0.5">
                  <span className="text-[10px] text-[var(--gray-500)]">Preço estimado</span>
            ...
            <div className="flex flex-col gap-4 px-4 py-4 pl-5">
              <div className="flex items-center justify-between">
                ...
                <button type="button" disabled={!canAdvance()} onClick={() => setStep(2)}
                  className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-indigo-300 bg-indigo-50 px-3 text-[11px] font-semibold text-indigo-700 transition hover:bg-indigo-100 disabled:opacity-40 dark:border-indigo-700/40 dark:bg-indigo-900/20 dark:text-indigo-300">
                  ...
                  {PRIORITY_CHOICES.map(p => (
                    <button key={p.value} type="button"
                      ...
                      className={`rounded-lg border px-2.5 py-2 text-center text-[11px] font-semibold transition
                        ${priority === p.value
                          ? "border-indigo-400 bg-indigo-50 text-indigo-700 dark:border-indigo-600/60 dark:bg-indigo-900/30 dark:text-indigo-300"
                          : "border-white/30 bg-white/20 text-[var(--gray-500)] hover:border-indigo-200 dark:border-white/10 dark:bg-white/[0.03]"}`}>
                      {p.label}
                    </button>
                  ))}
                </div>
              ...
              {totalPrice > 0 && (
                <div className="flex items-center justify-between rounded-lg border border-indigo-200/50 bg-indigo-50/40 px-3 py-2 dark:border-indigo-700/20 dark:bg-indigo-900/10">
                  <span className="text-[11px] text-[var(--gray-500)]">Preço estimado (procedimentos)</span>
                  <span className="text-[13px] font-semibold text-indigo-700 dark:text-indigo-300">{totalPrice.toLocaleString("pt-PT")} MT</span>
                </div>
              )}
            </div>
          </section>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => setStep(3)}
                    className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-indigo-300 bg-indigo-50 px-3 text-[11px] font-semibold text-indigo-700 transition hover:bg-indigo-100 dark:border-indigo-700/40 dark:bg-indigo-900/20 dark:text-indigo-300">
                    Próximo <ArrowRight size={11} />
                  </button>
                </div>
              </div>
        ...
        {step === 3 && (
          <section className={`relative ${GLASS}`}>
            <span className="absolute left-0 top-0 h-full w-1 rounded-l-xl bg-violet-400" />
            <div className="flex flex-col gap-4 px-4 py-4 pl-5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--gray-500)]">
                  ...
                  <div ref={teamRef}>
                    {!addingTeam ? (
                      <button type="button" onClick={() => { setAddingTeam(true); setTeamQuery("") }}
                        className="inline-flex items-center gap-1.5 rounded-lg border border-dashed border-[var(--gray-300)] px-3 py-1.5 text-[11px] text-[var(--gray-500)] hover:border-indigo-400 hover:text-indigo-600 dark:border-white/20">
                        + Adicionar membro
                      </button>
                    ) : (
                      <div className="rounded-lg border border-indigo-200/50 bg-indigo-50/30 p-2.5 dark:border-white/10 dark:bg-white/[0.03]">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" disabled={saving || !patient || procedures.length === 0} onClick={submit}
                    className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-indigo-300 bg-indigo-500 px-4 text-[11px] font-semibold text-white transition hover:bg-indigo-600 disabled:opacity-40">
              ...
              <div className="rounded-lg border border-white/30 bg-white/20 px-1 dark:border-white/10 dark:bg-white/[0.03]">
                ...
                <div className="px-3 pb-3">
                  ...
                  <ReviewRow label="Procedimentos" value={procedures.length > 0
                    ? <span className="flex flex-wrap gap-1">{procedures.map(p => <span key={p.id} className="rounded bg-indigo-100 px-1.5 py-0.5 text-[10px] text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300">{p.name}</span>)}</span>
                    : null} />
                  <ReviewRow label="Prioridade" value={PRIORITY_CHOICES.find(p => p.value === priority)?.label} />
                  <ReviewRow label="Tipo de cirurgia" value="Grande cirurgia" />
