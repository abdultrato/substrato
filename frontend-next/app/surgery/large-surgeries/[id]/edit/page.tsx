...
const PROCEDURE_SURGERY_TYPE_LABELS: Record<string, string> = {
  PEQUENA: "Pequena", GRANDE: "Grande", AMBAS: "Ambas",
}
...
function SearchSelect({ label, placeholder, endpoint, labelField = "name", value, initialLabel = "", onChange }: {
  label: string; placeholder: string; endpoint: string; labelField?: string
  value: number | null; initialLabel?: string; onChange: (id: number | null, label: string) => void
}) {
  ...
  useEffect(() => { if (initialLabel) setSelectedLabel(initialLabel) }, [initialLabel])
  ...
  function select(item: any) {
    ...
    setSelectedLabel(lbl); onChange(item.id, lbl); setOpen(false); setQuery("")
  }

  function clear(e: React.MouseEvent) {
    e.stopPropagation(); setSelectedLabel(""); onChange(null, "")
  }

  return (
    <FieldRow label={label}>
      <div ref={ref}>
        <div className={`${inputCls} flex cursor-pointer items-center justify-between gap-1`} onClick={() => setOpen(o => !o)}>
          {value && selectedLabel
        ...
        <DropdownPortal anchorRef={ref} portalRef={portalRef} open={open}>
          ...
          <div className="max-h-48 overflow-y-auto">
            {results.length === 0
              ...
              : results.map(item => (
                <div key={item.id}
                  className={`cursor-pointer px-3 py-1.5 text-[12px] hover:bg-indigo-50 dark:hover:bg-white/10 ${item.id === value ? "font-semibold text-indigo-700 dark:text-indigo-300" : "text-[var(--text)]"}`}
          ...
          <div className="max-h-52 overflow-y-auto">
            {results.length === 0
              ...
              : results.map(item => {
                ...
                return (
                  <div key={item.id}
                    className={`flex cursor-pointer items-center gap-2 px-3 py-1.5 text-[12px] hover:bg-indigo-50 dark:hover:bg-white/10 ${sel ? "bg-indigo-50 dark:bg-indigo-900/20" : ""}`}
...
function TeamMemberRow({ member, onRemove, onChangeRole }: {
  member: { tempId: string; employeeId: number | null; employeeName: string; role: string }
  onRemove: () => void; onChangeRole: (role: string) => void
}) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-white/20 bg-white/20 px-2 py-1.5 dark:bg-white/[0.03]">
      ...
      <select className="rounded border border-white/20 bg-transparent px-1.5 py-0.5 text-[11px] text-[var(--gray-500)] focus:outline-none"
        value={member.role} onChange={e => onChangeRole(e.target.value)}>
        {TEAM_ROLES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
      </select>
      <button type="button" onClick={onRemove} className="text-[var(--gray-400)] hover:text-rose-500"><Trash2 size={11} /></button>
    </div>
  )
}
...
export default function LargeSurgeryEditPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const d = await apiFetch<Record<string, any>>(`/surgery/large_surgery/${id}/`)
      ...
      if (procedureIds.length) {
        ...
        setProcedures(procedureIds.map((pid: number) => {
      ...
      await apiFetch<any>(`/surgery/large_surgery/${id}/sync-consumptions/`, { method: "POST" }).catch(() => {})
      ...
      for (const s of surgeonNames) {
        if (s.id) seen.set(s.id, { tempId: String(s.id), employeeId: s.id, employeeName: s.name, role: "SURGEON" })
      }
  ...
  async function confirmAdd() {
    ...
    try {
      ...
      setConsumptions(prev => [...prev, { id: res.id, product: pending.id, product_name: pending.name, quantity: String(qty), unit_cost: pending.sale_price }])
    } catch { }
  ...
  const save = useCallback(async () => {
    ...
    try {
      await apiFetch(`/surgery/large_surgery/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          patient,
      ...
      setSuccess(true)
      setTimeout(() => router.push(`/surgery/large-surgeries/${id}`), 800)
    } catch (e: any) {
        ...
        <section className={`relative overflow-hidden ${GLASS}`}>
          ...
          <div className="flex h-full items-center justify-between gap-3 px-4 py-3 pl-5">
            <div className="min-w-0">
              ...
              <h1 className="mt-0.5 font-display text-base font-semibold text-foreground">Editar grande cirurgia</h1>
            </div>
            <div className="flex shrink-0 items-center gap-3">
              ...
              <button onClick={save} disabled={saving || success}
                className="inline-flex h-7 items-center gap-1.5 rounded-md border border-indigo-300 bg-indigo-50 px-3 text-[11px] font-semibold text-indigo-700 transition hover:bg-indigo-100 disabled:opacity-50 dark:border-indigo-700/40 dark:bg-indigo-900/20 dark:text-indigo-300">
          ...
          <div style={{ breakInside: "avoid", marginBottom: "0.75rem" }}>
            <SurfaceCard title="1 · Paciente" icon={<User size={13} />} accent="bg-sky-400">
              <SearchSelect label="Paciente *" placeholder="Pesquisar paciente..."
                ...
                value={patient} initialLabel={patientLabel}
          ...
          <div style={{ breakInside: "avoid", marginBottom: "0.75rem" }}>
            <SurfaceCard title="5 · Equipa cirúrgica" icon={<Users size={13} />} accent="bg-violet-400">
              <div className="flex flex-col gap-2">
                ...
                {team.map(m => (
                  <TeamMemberRow key={m.tempId} member={m}
                ...
                <div ref={teamRef} className="relative">
                  {!addingTeam ? (
                    <button type="button" onClick={() => { setAddingTeam(true); setTeamQuery("") }}
                      className="inline-flex items-center gap-1.5 rounded-md border border-dashed border-[var(--gray-300)] px-3 py-1.5 text-[11px] text-[var(--gray-500)] hover:border-indigo-400 hover:text-indigo-600">
                    ...
                    <div className="rounded-lg border border-white/30 bg-white/40 p-2 backdrop-blur-sm dark:bg-white/[0.06]">
                      ...
                      <div ref={teamInputRef}>
                        <input autoFocus className={`${inputCls} mb-1`} placeholder="Pesquisar profissional..."
                        ...
                        <div className="max-h-96 overflow-y-auto">
                          {teamResults.filter(r => !team.some(m => m.employeeId === r.id)).length === 0
                            ...
                            : teamResults.filter(r => !team.some(m => m.employeeId === r.id)).map(emp => (
                              <div key={emp.id}
                                className="flex cursor-pointer items-center gap-2 px-3 py-1.5 text-[12px] hover:bg-indigo-50 dark:hover:bg-white/10"
