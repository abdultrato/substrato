...
function statusAccent(s: string) {
  ...
  if (["AUTHORIZED", "AGENDADA"].includes(s)) return "bg-indigo-400"
  return "bg-amber-400"
}
...
export default function LargeSurgeryDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [res, cons] = await Promise.all([
        apiFetch<Record<string, any>>(`/surgery/large_surgery/${id}/`, { clientCache: safeRefreshToken === 0 }),
  ...
  const markDone = useCallback(async () => {
    ...
    setMarking(true)
    try {
      const updated = await apiFetch<Record<string, any>>(`/surgery/large_surgery/${id}/`, {
                ...
                {procedureNames.length > 0
                  ? procedureNames.map((p, i) => (
                    <div key={i} className="flex items-center gap-2 rounded-lg border border-white/20 bg-white/20 px-2.5 py-1.5 dark:bg-white/[0.03]">
                      <Scissors size={11} className="shrink-0 text-indigo-400" />
