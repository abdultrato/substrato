...
export default function SmallSurgeriesDetailPage() {
  ...
  const endpointBase = "/surgery/small-surgeries/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["surgery", "small-surgeries", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar SmallSurgeries</div>;
  if (!item) return <div>SmallSurgeries não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `SmallSurgeries #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar SmallSurgeries"
            onSuccess={() => {
              setIsEditing(false);
