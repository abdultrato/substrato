...
export default function SmallSurgeriesListPage() {
  ...
  const load = useCallback(async (q: string, status: string | null, num: string) => {
    ...
    try {
      ...
      const d = await apiFetch<any>(`/surgery/small_surgery/?${p}`);
          ...
          <div className="px-3 py-2 pl-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <Link href="/surgery/small-surgeries/new"
                    ...
                    {STATUS_FILTER_OPTIONS.map(s => {
                      ...
                      return (
                        <button key={s} type="button"
                          ...
                          <span className={`h-2 w-2 shrink-0 rounded-full ${colorCls.replace(/text-\S+/, "").trim()} ${STATUS_COLOR[s]?.match(/bg-\S+/)?.[0] ?? "bg-gray-300"}`} />
