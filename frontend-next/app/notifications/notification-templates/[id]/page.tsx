...
export default function NotificationTemplatesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/notifications/notification-templates/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["notifications", "notification-templates", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar NotificationTemplates</div>;
  if (!item) return <div>NotificationTemplates não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `NotificationTemplates #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar NotificationTemplates"
            onSuccess={() => {
              setIsEditing(false);
