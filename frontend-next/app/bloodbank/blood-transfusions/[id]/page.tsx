...
export default function BloodTransfusionsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/bloodbank/blood-transfusions/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["bloodbank", "blood-transfusions", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar BloodTransfusions</div>;
  if (!item) return <div>BloodTransfusions não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `BloodTransfusions #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar BloodTransfusions"
            onSuccess={() => {
              setIsEditing(false);
