...
export default function BloodDonationsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/bloodbank/blood-donations/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["bloodbank", "blood-donations", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar BloodDonations</div>;
  if (!item) return <div>BloodDonations não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `BloodDonations #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar BloodDonations"
            onSuccess={() => {
              setIsEditing(false);
