...
export default function BloodStockMovementsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/bloodbank/blood-stock-movements/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["bloodbank", "blood-stock-movements", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar BloodStockMovements</div>;
  if (!item) return <div>BloodStockMovements não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `BloodStockMovements #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar BloodStockMovements"
            onSuccess={() => {
              setIsEditing(false);
