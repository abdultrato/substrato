...
export default function BloodUnitsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/bloodbank/blood-units/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["bloodbank", "blood-units", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar BloodUnits</div>;
  if (!item) return <div>BloodUnits não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `BloodUnits #${id}`}</h1>
