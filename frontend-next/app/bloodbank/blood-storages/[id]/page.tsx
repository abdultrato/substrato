...
export default function BloodStoragesDetailPage() {
  useAuthGuard();
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["bloodbank", "blood-storages", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `BloodStorages #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar BloodStorages"
            onSuccess={() => {
              setIsEditing(false);
