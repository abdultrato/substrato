...
export default function InvoiceHistoriesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/billing/invoice-histories/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["billing", "invoice-histories", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar InvoiceHistories</div>;
  if (!item) return <div>InvoiceHistories não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `InvoiceHistories #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar InvoiceHistories"
            onSuccess={() => {
              setIsEditing(false);
