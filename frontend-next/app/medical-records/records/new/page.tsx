...
function CreateRecordInner() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (prescription) body.prescription = prescription;
      if (medicalReport) body.medical_report = medicalReport;

      const result = await apiFetch<{ id: number }>("/medical_records/record/", {
        method: "POST",
        body: JSON.stringify(body),
      });
      router.push(`/medical-records/records/${result.id}`);
    } catch (reason: any) {
      setError(reason?.message || "Erro ao criar cardex.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <section className={`relative overflow-hidden ${GLASS}`}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 pl-5">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="min-w-0">
                <h1 className="text-lg font-bold leading-tight text-foreground">Novo cardex</h1>
                <p className="text-[11px] text-muted-foreground">Registo de prontuário clínico</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Link href="/medical-records/records"
...
export default function MedicalRecordsRecordsCreatePage() {
  return (
