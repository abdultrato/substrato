...
export default function LabPanelsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (filterProfile) query.profile_type = filterProfile;
      const res = await apiFetchList<LabTestPanel>("/clinical_laboratory/panel/", {
      ...
      setError(e?.message || "Erro ao carregar painéis.");
    } finally {
      setLoading(false);
    }
  }, [page, debouncedSearch, filterActive, filterProfile, safeRefreshToken]);

  useEffect(() => { setPage(1); }, [debouncedSearch, filterActive, filterProfile]);
  useEffect(() => { load(); }, [load]);
      ...
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 py-20 text-center">
            ...
            <Link href="/clinical-laboratory/panels/new"
              ...
              <Plus size={13} /> Criar primeiro painel
            </Link>
          </div>
        ) : (
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {panels.map((p) => (
              <Link key={p.id} href={`/clinical-laboratory/panels/${p.id}`}
                ...
                <div className="flex flex-1 flex-col gap-2 px-4 py-3 pl-5">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-[10px] font-mono text-muted-foreground">{p.code || p.custom_id}</p>
                  ...
                  {/* Footer: tests count + price */}
                  <div className="flex items-center justify-between border-t border-border/40 pt-1.5">
                    <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                      <FlaskConical size={11} /> {p.tests?.length ?? 0} exame{(p.tests?.length ?? 0) !== 1 ? "s" : ""}
                    </span>
                    <span className="text-xs font-semibold text-foreground">{fmtPrice(p.package_price)}</span>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between gap-2 pt-1">
            <p className="text-[11px] text-muted-foreground">
              Página {page} de {totalPages} · {total} painéis
            </p>
