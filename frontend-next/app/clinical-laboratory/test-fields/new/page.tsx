...
function NewTestFieldForm() {
  ...
  function validate() {
    ...
    if (!testId) e.test = "Exame não especificado.";
    setErrors(e);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/clinical-laboratory/tests/${testId}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao criar campo.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/20 bg-white/30 px-4 py-3 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold leading-tight text-foreground">Novo campo</h1>
                <p className="text-[11px] text-muted-foreground">
                  {testId ? `Exame #${testId}` : "Campo de exame laboratorial"}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button type="submit" disabled={saving}
                ...
                {saving ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />} Guardar campo
              </button>
            </div>
          </div>
        </div>
