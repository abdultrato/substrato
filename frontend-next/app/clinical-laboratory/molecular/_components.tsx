...
import { CardFooter, Pill, StatusPill, fmtDate } from "@/components/clinical-laboratory/ResourceCardList";
                ...
                <div className="grid min-h-0 flex-1 grid-cols-[repeat(auto-fit,minmax(min(100%,260px),1fr))] content-start gap-1 overflow-y-auto pr-1">
                  {filteredPending.map((item) => (
                    ...
                    >
                      ...
                      <div className="mb-1.5 flex items-start justify-between gap-1.5">
                        <div className="min-w-0">
                          ...
                          <p className="truncate font-mono text-[11px] font-semibold text-muted-foreground">
                            {item.order_custom_id || item.order_item_custom_id || "Pedido sem código"}
                          </p>
                        </div>
                      ...
                      <CardFooter
                        left={<>{item.test_name || item.test_code || "Exame molecular"}</>}
                  ...
                  {filteredResults.map((m) => (
                    ...
                    >
                      ...
                      <div className="mb-1.5 flex items-start justify-between gap-1.5">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold leading-snug text-foreground">
                            {m.patient_name || "Paciente não identificado"}
                          </p>
                          <p className="truncate font-mono text-[11px] font-semibold text-muted-foreground">
                            {m.order_custom_id || m.custom_id || "Pedido sem código"}
                          </p>
                        </div>
                        <StatusPill tone={DETECTION_TONE[m.detection] ?? "gray"}>{DETECTION_LABELS[m.detection] ?? m.detection}</StatusPill>
                      </div>
                      <div className="flex flex-wrap gap-1">
                        <Pill tone="indigo">{ASSAY_LABELS[m.assay] ?? m.assay}</Pill>
                      ...
                      <CardFooter left={<>{m.custom_id || m.test_name || "Resultado molecular"}</>} right={m.performed_at ? `Realizado ${fmtDate(m.performed_at)}` : "—"} />
                    </Link>
                  ))}
                </div>
              ) : (
