...
export default function EditCompetencyPage() {
  ...
  const [relatedTestLabel, setRelatedTestLabel] = useState("");
  const [assessedBy,      setAssessedBy]      = useState<number | null>(null);
  const [assessedByLabel, setAssessedByLabel] = useState("");
  ...
  useEffect(() => {
    if (!id) return;
    setLoadingRec(true);
    apiFetch<any>(`/clinical_laboratory/competency/${id}/`)
      .then((rec) => {
        setStaff(rec.staff ?? null);
        setStaffLabel(rec.staff_display ?? "");
        setArea(rec.area ?? "");
        setRelatedTest(rec.related_test ?? null);
        setRelatedTestLabel(rec.related_test_display ?? "");
        setAssessedBy(rec.assessed_by ?? null);
        setAssessedByLabel(rec.assessed_by_display ?? "");
        setAssessmentDate(rec.assessment_date ?? "");
        setExpiryDate(rec.expiry_date ?? "");
        setStatus(rec.status ?? "AGENDADA");
        setNotes(rec.notes ?? "");
        setCustomId(rec.custom_id ?? "");
      })
      .catch((e) => setLoadError(e?.message ?? "Erro ao carregar avaliação."))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/clinical_laboratory/competency/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
      ...
      router.push(`/clinical-laboratory/quality-management/competencies/${id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao guardar avaliação.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={User} title="Colaborador" accent="bg-fuchsia-500">
            ...
            <Field label="Avaliado por">
              <RelationSelect
                value={assessedBy}
                onChange={(v, l) => { setAssessedBy(v); setAssessedByLabel(l); }}
                target={T_USER}
                placeholder="Pesquisar avaliador…"
                safeRefreshToken={safeRefreshToken}
                initialLabel={assessedByLabel}
              />
            </Field>
          </Card>
          ...
          <Card icon={Award} title="Competência" accent="bg-purple-500">
            <Field label="Atividade / competência" required error={errors.area}>
              <input
                ...
                placeholder="Ex: Pipetagem, coloração de Gram…"
            ...
            <Field label="Exame relacionado" hint="Opcional — exame do catálogo ligado a esta competência.">
              <RelationSelect
                value={relatedTest}
                onChange={(v, l) => { setRelatedTest(v); setRelatedTestLabel(l); }}
                target={T_LABTEST}
                placeholder="Pesquisar exame…"
                safeRefreshToken={safeRefreshToken}
                initialLabel={relatedTestLabel}
              />
            </Field>
          </Card>
