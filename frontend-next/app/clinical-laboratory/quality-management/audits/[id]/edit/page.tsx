...
export default function EditAuditPage() {
  ...
  useEffect(() => {
    if (!id) return;
    setLoadingRec(true);
    apiFetch<any>(`/clinical_laboratory/internal_audit/${id}/`)
      .then((rec) => {
        setCode(rec.code ?? "");
        setSectors((rec.sectors_display ?? []).map((s: any) => ({ id: s.id, label: s.label })));
        setAuditor(rec.auditor ?? null);
        setAuditorLabel(rec.auditor_display ?? "");
        setAuditDate(rec.audit_date ?? "");
        setScope(rec.scope ?? "");
        setCriteria(rec.criteria ?? "");
        setConclusion(rec.conclusion ?? "");
        setStatus(rec.status ?? "PLANEADA");
        setCustomId(rec.custom_id ?? "");
      })
      .catch((e) => setLoadError(e?.message ?? "Erro ao carregar auditoria."))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/clinical_laboratory/internal_audit/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
      ...
      router.push(`/clinical-laboratory/quality-management/audits/${id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao guardar auditoria.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={MapPin} title="Sectores auditados" accent="bg-blue-500">
            ...
            <Field label="Código interno">
              <div className="flex items-center gap-2 rounded-md border border-dashed border-border bg-muted/40 px-2.5 py-1.5 text-xs text-muted-foreground">
                ...
                <span className="font-mono">{code || "gerado automaticamente"}</span>
              </div>
            </Field>
          </Card>
          ...
          <Card icon={User} title="Auditor e data" accent="bg-cyan-500">
            <Field label="Auditor">
              <RelationSelect
                value={auditor}
                onChange={(v, l) => { setAuditor(v); setAuditorLabel(l); }}
                target={T_USER}
                placeholder="Pesquisar auditor…"
                safeRefreshToken={safeRefreshToken}
                initialLabel={auditorLabel}
              />
            </Field>
