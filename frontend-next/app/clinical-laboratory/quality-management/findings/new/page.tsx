...
export default function NewFindingPage() {
  ...
  const presetAudit = params.get("audit");

  const [audit,        setAudit]        = useState<number | null>(presetAudit ? Number(presetAudit) : null);
  ...
  // se veio audit por query, pré-carrega o label
  useEffect(() => {
    if (!presetAudit) return;
    apiFetch<any>(`/clinical_laboratory/internal_audit/${presetAudit}/`)
      .then((a) => setAuditLabel(a.area || a.code || a.custom_id || ""))
      .catch(() => {});
  }, [presetAudit]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const created = await apiFetch<{ id: number }>("/clinical_laboratory/audit_finding/", {
        method: "POST",
        body: JSON.stringify({
          audit,
      ...
      router.push(`/clinical-laboratory/quality-management/findings/${created.id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao criar achado.");
      setSaving(false);
    }
  }
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="text-base font-bold leading-tight text-foreground">
                {description.trim().slice(0, 60) || "Novo achado de auditoria"}
              </h1>
