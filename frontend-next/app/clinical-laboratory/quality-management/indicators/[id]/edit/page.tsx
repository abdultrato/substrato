...
import { ArrowLeft, FileText, GaugeCircle, Loader2, Save, Target, TrendingUp } from "lucide-react";
...
export default function EditQualityIndicatorPage() {
  useAuthGuard();
  ...
  useEffect(() => {
    ...
    apiFetch<Indicator>(`/clinical_laboratory/quality_indicator/${id}/`)
      .then((rec) => {
        ...
        setTargetValue(rec.target_value === null || rec.target_value === undefined ? "" : String(rec.target_value));
        setCurrentValue(rec.current_value === null || rec.current_value === undefined ? "" : String(rec.current_value));
        setPeriod(rec.period ?? "");
        setStatus(rec.status ?? "NAO_MEDIDO");
        setCustomId(rec.custom_id ?? "");
      })
      .catch((e) => setLoadError(e?.message ?? "Erro ao carregar indicador."))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/clinical_laboratory/quality_indicator/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          name: name.trim(),
          sector,
      ...
      router.push(`/clinical-laboratory/quality-management/indicators/${id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao guardar indicador.");
    } finally {
      setSaving(false);
    }
  }
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                ...
                <span className="font-mono text-[9px]">{customId}</span><span>/</span>
                <span className="font-medium text-foreground">Editar</span>
              </div>
              <h1 className="text-base font-bold leading-tight text-foreground">{name.trim() || "Editar indicador"}</h1>
              {currentStatus && (
