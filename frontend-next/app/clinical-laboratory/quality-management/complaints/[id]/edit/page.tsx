...
export default function EditComplaintPage() {
  ...
  useEffect(() => {
    if (!id) return;
    setLoadingRec(true);
    apiFetch<any>(`/clinical_laboratory/complaint/${id}/`)
      .then((rec) => {
        setCode(rec.code ?? "");
        setSource(rec.source ?? "");
        setReceivedAt(toLocalInput(rec.received_at));
        setDescription(rec.description ?? "");
        setInvestigation(rec.investigation ?? "");
        setResponse(rec.response ?? "");
        setStatus(rec.status ?? "RECEBIDA");
        setCustomId(rec.custom_id ?? "");
      })
      .catch((e) => setLoadError(e?.message ?? "Erro ao carregar reclamação."))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/clinical_laboratory/complaint/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          code:          code.trim(),
          source:        source.trim(),
      ...
      router.push(`/clinical-laboratory/quality-management/complaints/${id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao guardar reclamação.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <AppLayout requiredGroups={EDIT_GROUPS}>
      <form onSubmit={handleSubmit} noValidate className="mx-auto w-[90%] space-y-2">
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          {/* Investigação */}
          <Card icon={FileText} title="Investigação" accent="bg-sky-500">
          ...
          {/* Resposta */}
          <Card icon={Reply} title="Resposta ao reclamante" accent="bg-indigo-500">
            <Field label="Resposta" hint="Resposta dada e ações comunicadas.">
              <textarea
                value={response}
