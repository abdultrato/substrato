...
const VACCINE_SUGGESTIONS = [
  ...
  "BCG (Tuberculose)",
  "MMR (Sarampo/Parotidite/Rubéola)",
  "Varicela",
  "Febre Tifóide",
  "Febre Amarela",
...
// ── VaccineInput — text with suggestions ─────────────────────────────────────
...
export default function NewVaccinationRecordPage() {
  useAuthGuard();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const data = await apiFetch<{ id?: number }>("/clinical_laboratory/vaccination/", {
        method: "POST",
        body: JSON.stringify({
          staff,
      ...
      router.push(`/clinical-laboratory/biosafety/vaccination/${data.id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao registar vacinação.");
    ...
    <AppLayout requiredGroups={CREATE_GROUPS}>
      <form onSubmit={handleSubmit} noValidate className="mx-auto w-[90%] space-y-2">
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/20 bg-white/30 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
          ...
          <span className={`absolute inset-y-0 left-0 w-1 rounded-l-xl ${currentStatus?.bar ?? "bg-teal-500"}`} />
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="text-base font-bold leading-tight text-foreground">
                {vaccine.trim() ? `Vacina — ${vaccine}` : "Novo registo de vacinação"}
              </h1>
              <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                {currentStatus && (
                  <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${currentStatus.color}`}>
                    <Shield size={8} className="mr-0.5" />{currentStatus.label}
                  </span>
                )}
                {doseNumber && (
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={CalendarDays} title="Datas e comprovativo" accent="bg-sky-500">
            ...
            <Field label="Comprovativo / Número de lote" hint="Referência do documento ou lote da vacina">
