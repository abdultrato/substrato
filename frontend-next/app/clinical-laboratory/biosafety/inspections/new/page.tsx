    ...
    <div className="space-y-2">
      ...
      {items.length > 0 && (
        <div className="space-y-0.5">
          <div className="flex items-center justify-between text-[10px] text-muted-foreground">
            <span>{checked}/{items.length} itens verificados</span>
        ...
        {items.map((item) => (
          <div key={item.id}
            ...
            <button type="button" onClick={() => toggle(item.id)}
              ...
              {item.checked && <svg viewBox="0 0 12 12" fill="none" className="h-2.5 w-2.5"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}
            </button>
            <span className={`flex-1 text-[11px] ${item.checked ? "line-through text-muted-foreground" : "text-foreground"}`}>
              {item.label}
            </span>
            <button type="button" onClick={() => remove(item.id)}
              className="text-muted-foreground/50 transition hover:text-red-500">
              <Trash2 size={10} />
            </button>
          </div>
        ))}
      ...
      <div className="relative">
        <div className="flex gap-1.5">
          ...
          />
          <button type="button" onClick={() => addItem(newLabel)}
        ...
        {showTemplates && (
          <div className="absolute left-0 right-0 z-[999] mt-1 overflow-hidden rounded-lg border border-border bg-card shadow-lg">
            <p className="border-b border-border/50 px-3 py-1.5 text-[10px] font-semibold text-muted-foreground">
              Itens predefinidos de biossegurança
            </p>
...
export default function NewBiosafetyInspectionPage() {
  useAuthGuard();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const checklistPayload = checklist.map(({ label, checked }) => ({ label, checked }));
      const data = await apiFetch<{ id?: number }>("/clinical_laboratory/biosafety_inspection/", {
        method: "POST",
        body: JSON.stringify({
          area:            area.trim(),
      ...
      router.push(`/clinical-laboratory/biosafety/inspections/${data.id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao criar inspecção.");
    } finally {
      setSaving(false);
    }
  }
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="text-base font-bold leading-tight text-foreground">
                {area.trim() ? `Inspecção — ${area}` : "Nova inspecção de biossegurança"}
              </h1>
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={MapPin} title="Área inspeccionada" accent="bg-emerald-500">
            <Field label="Área / local" required error={errors.area}
              hint="Ex.: Laboratório de Microbiologia, Sala de colheitas, Corredor B">
          ...
          <div className="lg:col-span-2">
            <Card icon={AlertTriangle} title="Achados e observações" accent="bg-amber-400">
              <Field label="Descrição dos achados / não conformidades detectadas"
                hint="Descreva os problemas identificados durante a inspecção, se aplicável.">
                <textarea value={findings}
                  onChange={(e) => setFindings(e.target.value)}
                  placeholder="Ex.: EPI danificado no sector de microbiologia; sinalização ausente na porta de entrada…"
                  rows={5}
