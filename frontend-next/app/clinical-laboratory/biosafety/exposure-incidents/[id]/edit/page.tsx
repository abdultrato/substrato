...
const STATUS_CHOICES = [
  ...
  { value: "SAUDE_OCUP", label: "Saúde ocupacional" },
      ...
      {value === null && (
        <div className="relative z-[999]">
          ...
          <input type="text" value={query}
            ...
            className="w-full rounded-md border border-border bg-background py-1.5 pl-7 pr-3 text-xs text-foreground outline-none transition focus:border-red-400 focus:ring-2 focus:ring-red-400/20"
          />
...
export default function EditExposureIncidentPage() {
  useAuthGuard();
  ...
  useEffect(() => {
    if (!id) return;
    apiFetch<Record<string, any>>(`/clinical_laboratory/exposure_incident/${id}/`)
      .then((d) => {
        setCustomId(d.custom_id ?? "");
        setStaff(d.staff ?? null);
        setStaffLabel(d.staff_detail?.name ?? "");
        setIncidentAt(d.incident_at ? new Date(d.incident_at).toISOString().slice(0, 16) : "");
        setExposureType(d.exposure_type ?? "");
        setStatus(d.status ?? "REPORTADO");
        setMaterialInvolved(d.material_involved ?? "");
        setBodySite(d.body_site ?? "");
        setActivity(d.activity ?? "");
        setImmediateAction(d.immediate_action ?? "");
        setReportedTo(d.reported_to ?? "");
        setRequiresFollowup(d.requires_medical_followup ?? true);
        setRootCause(d.root_cause ?? "");
        setContributingFactors(d.contributing_factors ?? "");
        setConclusion(d.conclusion ?? "");
        setInvestigatedBy(d.investigated_by ?? null);
        setInvestigatedByLabel(d.investigator_detail?.name ?? "");
        setInvestigatedAt(d.investigated_at ? new Date(d.investigated_at).toISOString().slice(0, 16) : "");
      })
      .catch(() => setSaveError("Erro ao carregar incidente."))
  ...
  function validate() {
    ...
    if (!incidentAt)   e.date  = "Data e hora obrigatórias.";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/clinical_laboratory/exposure_incident/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          staff,
      ...
      router.push(`/clinical-laboratory/biosafety/exposure-incidents/${id}`);
    } catch (err: any) {
      ...
      <form onSubmit={handleSubmit} noValidate className="mx-auto w-[90%] space-y-2">
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="text-base font-bold leading-tight text-foreground">
                {currentExpType ? `Editar — ${currentExpType.label}` : "Editar incidente de exposição"}
              </h1>
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={User} title="Colaborador e data do incidente" accent="bg-red-500">
            ...
            <Field label="Reportado a">
              <input type="text" value={reportedTo}
                onChange={(e) => setReportedTo(e.target.value)}
                placeholder="Supervisor ou responsável notificado"
                className={inputCls}
              />
            </Field>
          </Card>
          ...
          <Card icon={AlertTriangle} title="Tipo de exposição e estado" accent="bg-orange-500">
            <Field label="Tipo de exposição" required error={errors.type}>
              <div className="flex flex-wrap gap-1.5">
                {EXPOSURE_TYPE_CHOICES.map(({ value, label }) => {
                  const active = exposureType === value;
                  const clr = EXPOSURE_TYPE_COLOR[value] ?? "";
                  return (
                    <button key={value} type="button"
                      onClick={() => { setExposureType(value); setErrors((p) => ({ ...p, type: "" })); }}
          ...
          <Card icon={Activity} title="Detalhes da exposição" accent="bg-amber-400">
            ...
            <Field label="Local corporal afectado">
            ...
            <Field label="Actividade em curso">
              <input type="text" value={activity}
                onChange={(e) => setActivity(e.target.value)}
                placeholder="O que estava a ser feito quando ocorreu o incidente"
                className={inputCls}
              />
            </Field>
          </Card>
          ...
          <Card icon={Stethoscope} title="Acção imediata e acompanhamento" accent="bg-blue-500">
            <Field label="Acção imediata tomada">
              <textarea value={immediateAction}
                ...
                placeholder="Descreva as acções imediatas tomadas…"
                ...
                <div className="space-y-2">
                  <Field label="Investigado por">
                    <RelationSelect
                      ...
                      initialLabel={investigatedByLabel}
                      safeRefreshToken={safeRefreshToken}
                    />
                  </Field>
