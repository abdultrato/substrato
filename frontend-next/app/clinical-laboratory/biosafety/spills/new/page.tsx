...
const SPILL_TYPES = [
  { value: "BIOLOGICO", label: "Biológico",  icon: "🧫", bar: "bg-violet-500", chip: "border-violet-200 bg-violet-50 text-violet-700 dark:border-violet-700/40 dark:bg-violet-900/20 dark:text-violet-300" },
  { value: "QUIMICO",   label: "Químico",    icon: "⚗️",  bar: "bg-amber-500",  chip: "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-700/40 dark:bg-amber-900/20 dark:text-amber-300"   },
];

const LOCATIONS = [
  "Laboratório Clínico", "Banco de Sangue", "Sala de Colheitas", "Microbiologia",
  "Imunologia", "Bioquímica", "Hematologia", "Bloco Operatório",
  "Urgência", "UCI", "Corredor", "Armazém", "Estufa / Câmara fria",
  "Sala de descontaminação", "Farmácia",
];

const MATERIALS = [
  "Sangue total", "Soro", "Urina", "Fezes", "LCR", "Líquido pleural",
  "Cultura microbiológica", "Ácido sulfúrico", "Formol", "Hipoclorito",
  "Reagente de PCR", "Xilol", "Álcool etílico", "Solução de fixação",
  "Reagente Giemsa",
];

const DISINFECTION_METHODS = [
  "Hipoclorito de sódio 1%", "Hipoclorito de sódio 10%", "Álcool 70%",
  "Glutaraldeído 2%", "Fenol 5%", "Autoclavagem do material absorvente",
  "Aspiração + desinfecção", "Kit de derrame químico padrão",
  "Neutralização ácido-base + absorção",
];
...
export default function NewSpillPage() {
  useAuthGuard();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (exposureIncidentId)        body.exposure_incident   = exposureIncidentId;

      const res = await apiFetch<{ id: number }>("/clinical_laboratory/spill/", {
        method: "POST",
        body: JSON.stringify(body),
      });
      router.push(`/clinical-laboratory/biosafety/spills/${res.id}`);
    } catch (err: any) {
      setSaveError(err?.message || "Erro ao registar derrame.");
    } finally { setSaving(false); }
  }

  const activeType = SPILL_TYPES.find((t) => t.value === spillType)!;
      ...
      <form onSubmit={handleSubmit} noValidate className="mx-auto w-[90%] space-y-2">
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/20 bg-white/30 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl shadow-md ${spillType === "BIOLOGICO" ? "bg-gradient-to-br from-violet-500 to-purple-600 shadow-violet-500/30" : "bg-gradient-to-br from-amber-500 to-orange-600 shadow-amber-500/30"}`}>
              {spillType === "BIOLOGICO" ? <FlaskConical size={20} className="text-white" /> : <Zap size={20} className="text-white" />}
            </div>
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="text-base font-bold leading-tight text-foreground">
                {location.trim() ? `Derrame ${activeType.label} — ${location}` : "Novo registo de derrame"}
              </h1>
              <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${activeType.chip}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="submit" disabled={saving}
                className={`inline-flex h-8 items-center gap-1.5 rounded-lg px-4 text-xs font-semibold text-white shadow-md transition disabled:opacity-50 ${spillType === "BIOLOGICO" ? "bg-gradient-to-r from-violet-600 to-purple-600 shadow-violet-500/30 hover:from-violet-700 hover:to-purple-700" : "bg-gradient-to-r from-amber-600 to-orange-600 shadow-amber-500/30 hover:from-amber-700 hover:to-orange-700"}`}>
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={MapPin} title="Tipo e localização" accent={activeType.bar}>
            ...
            <Field label="Local / Área" required error={errors.location}>
              <SuggestInput value={location} onChange={(v) => { setLocation(v); if (v.trim()) setErrors((p) => ({ ...p, location: "" })); }}
                suggestions={LOCATIONS} placeholder="Ex.: Laboratório Clínico, Bloco Operatório…" zIndex="z-[997]" />
            </Field>
          ...
          {/* Material + Volume */}
          <Card icon={FlaskConical} title="Material e volume" accent="bg-orange-400">
            <Field label="Material envolvido" hint="Substância ou produto derramado">
            ...
            <Field label="Volume estimado" hint="Quantidade aproximada derramada">
            ...
            <Field label="Método de desinfeção utilizado">
              <SuggestInput value={disinfectionMethod} onChange={setDisinfectionMethod}
                suggestions={DISINFECTION_METHODS} placeholder="Ex.: Hipoclorito 1%, Kit de derrame…" zIndex="z-[995]" />
            </Field>
          </Card>
          ...
          <div className="lg:col-span-2">
            <Card icon={Shield} title="Ação imediata e exposição" accent="bg-red-500">
              <div className="grid gap-3 sm:grid-cols-2">
                <Field label="Ação imediata tomada" hint="Descreva os passos de contenção e limpeza">
                  <textarea value={immediateAction}
                    onChange={(e) => setImmediateAction(e.target.value)}
                    rows={4} placeholder="Ex.: Área isolada, material absorvente aplicado, descartado como resíduo biológico…"
