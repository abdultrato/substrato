...
function SuggestInput({
  ...
  placeholder?: string; icon?: React.ElementType; zIndex?: string;
}) {
  const [open, setOpen] = useState(false);
...
export default function EditDecontaminationPage() {
  useAuthGuard();
  ...
  useEffect(() => {
    if (!id) return;
    apiFetch<Record<string, any>>(`/clinical_laboratory/decontamination/${id}/`)
      .then((d) => {
        setCustomId(d.custom_id ?? "");
        setArea(d.area ?? "");
        setEquipment(d.equipment ?? "");
        setDisinfectant(d.disinfectant ?? "");
        setConcentration(d.concentration ?? "");
        setReason(d.reason ?? "");
        setPerformedBy(d.performed_by ?? null);
        setPerformedByLabel(d.performed_by_detail?.name ?? "");
        setPerformedAt(d.performed_at ? new Date(d.performed_at).toISOString().slice(0, 16) : "");
        setVerifiedBy(d.verified_by ?? null);
        setVerifiedByLabel(d.verified_by_detail?.name ?? "");
      })
  ...
  function validate() {
    ...
    if (!performedBy)         e.performed_by = "Responsável obrigatório.";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/clinical_laboratory/decontamination/${id}/`, {
        method: "PATCH",
        body: JSON.stringify({
          area:          area.trim(),
      ...
      router.push(`/clinical-laboratory/biosafety/decontamination/${id}`);
    } catch (err: any) {
    ...
    <AppLayout requiredGroups={EDIT_GROUPS}>
      <form onSubmit={handleSubmit} noValidate className="mx-auto w-[90%] space-y-2">
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/20 bg-white/30 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/10">
          ...
          <span className={`absolute inset-y-0 left-0 w-1 rounded-l-xl ${verifiedBy ? "bg-emerald-500" : "bg-cyan-500"}`} />
          ...
          <div className="relative flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="text-base font-bold leading-tight text-foreground">
                {area.trim() ? `Editar — ${area}` : "Editar registo de descontaminação"}
              </h1>
        ...
        <div className="grid gap-2 lg:grid-cols-2">
          ...
          <Card icon={MapPin} title="Área e equipamento" accent="bg-cyan-500">
            ...
            <Field label="Equipamento / Superfície" hint="Deixar em branco se for toda a área">
              <SuggestInput value={equipment}
                ...
                placeholder="Ex.: Câmara de fluxo laminar"
          ...
          <Card icon={FlaskConical} title="Produto desinfectante" accent="bg-sky-500">
            ...
            <Field label="Concentração / Diluição">
