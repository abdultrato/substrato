...
function ReportCard({
  ...
  return (
    <article className="rounded border border-[var(--border)] bg-[var(--card)] p-3 shadow-sm">
      ...
      <div className="mt-2.5 flex items-center justify-end gap-2 border-t border-[var(--border)] pt-2">
        <button
          type="button"
...
export default function LabReportsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const { items } = await apiFetchList<LabRequest>("/clinical/labrequest/?fase=laudos", {
        page: 1,
        pageSize: 200,
        clientCache: false,
      })
  ...
  async function handleNotify(row: LabRequest) {
    ...
    try {
      ...
      setFeedback(`Notificação enviada ao paciente de ${row.custom_id}.`)
    } catch (e: any) {
  ...
  return (
    <AppLayout>
      <div className="mx-auto w-full max-w-3xl space-y-3">
        <PageHeader
          title="Laudos"
          subtitle="Resultados validados — acesso a PDF, exportação e notificação ao paciente."
        />
