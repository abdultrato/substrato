...
export default function ProcedureMaterialValuesDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["nursing", "procedure-material-values", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ProcedureMaterialValues #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ProcedureMaterialValues"
            onSuccess={() => {
              setIsEditing(false);
