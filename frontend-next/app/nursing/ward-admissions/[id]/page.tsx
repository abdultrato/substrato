...
export default function WardAdmissionsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/nursing/ward-admissions/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["nursing", "ward-admissions", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar WardAdmissions</div>;
  if (!item) return <div>WardAdmissions não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `WardAdmissions #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar WardAdmissions"
            onSuccess={() => {
              setIsEditing(false);
