...
export default function NursingEvolutionsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/nursing/nursing-evolutions/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["nursing", "nursing-evolutions", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar NursingEvolutions</div>;
  if (!item) return <div>NursingEvolutions não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `NursingEvolutions #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar NursingEvolutions"
            onSuccess={() => {
              setIsEditing(false);
