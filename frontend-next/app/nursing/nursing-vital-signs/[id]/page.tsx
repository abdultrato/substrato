...
export default function NursingVitalSignsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["nursing", "nursing-vital-signs", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `NursingVitalSigns #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar NursingVitalSigns"
            onSuccess={() => {
              setIsEditing(false);
