...
export default function NursingProceduresPage() {
  ...
  const canCreate = hasWriteContract("/nursing/procedures/", "post");
  const canEdit = hasWriteContract("/nursing/procedures/{id}/", "put");
  const pageSize = 20;

  const { data, isLoading, error } = useQuery({
    queryKey: ["nursing", "procedures", page, search],
    queryFn: async () => {
      return await apiFetchList<any>("/nursing/procedures/", { page, pageSize, query: { search: search || undefined } });
    },
