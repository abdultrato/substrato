...
export default function ProcedureCatalogsDetailPage() {
  ...
  const endpointBase = "/nursing/procedure-catalogs/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["nursing", "procedure-catalogs", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ProcedureCatalogs</div>;
  if (!item) return <div>ProcedureCatalogs não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ProcedureCatalogs #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ProcedureCatalogs"
            onSuccess={() => {
              setIsEditing(false);
