...
export default function ProcedureMaterialsDetailPage() {
  ...
  const endpointBase = "/nursing/procedure-materials/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["nursing", "procedure-materials", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ProcedureMaterials</div>;
  if (!item) return <div>ProcedureMaterials não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ProcedureMaterials #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ProcedureMaterials"
            onSuccess={() => {
              setIsEditing(false);
