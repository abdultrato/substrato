...
export default function ProductCategoriesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/pharmacy/product-categories/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["pharmacy", "product-categories", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ProductCategories</div>;
  if (!item) return <div>ProductCategories não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ProductCategories #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ProductCategories"
            onSuccess={() => {
              setIsEditing(false);
