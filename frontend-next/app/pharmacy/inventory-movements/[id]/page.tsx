...
export default function InventoryMovementsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["pharmacy", "inventory-movements", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `InventoryMovements #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar InventoryMovements"
            onSuccess={() => {
              setIsEditing(false);
