...
export default function MaterialRequisitionItemsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["pharmacy", "material-requisition-items", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `MaterialRequisitionItems #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar MaterialRequisitionItems"
            onSuccess={() => {
              setIsEditing(false);
