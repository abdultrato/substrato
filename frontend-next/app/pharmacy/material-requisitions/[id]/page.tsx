...
export default function MaterialRequisitionsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["pharmacy", "material-requisitions", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `MaterialRequisitions #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar MaterialRequisitions"
            onSuccess={() => {
              setIsEditing(false);
