...
export default function SaleItemsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/pharmacy/sale-items/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["pharmacy", "sale-items", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar SaleItems</div>;
  if (!item) return <div>SaleItems não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `SaleItems #${id}`}</h1>
