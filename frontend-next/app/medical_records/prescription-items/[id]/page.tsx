...
export default function PrescriptionItemsDetailPage() {
  ...
  const endpointBase = "/medical_records/prescription-items/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["medical_records", "prescription-items", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar PrescriptionItems</div>;
  if (!item) return <div>PrescriptionItems não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `PrescriptionItems #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar PrescriptionItems"
            onSuccess={() => {
              setIsEditing(false);
