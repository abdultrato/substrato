...
export default function TenantUsagesDetailPage() {
  useAuthGuard();
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["tenants", "tenant-usages", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `TenantUsages #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar TenantUsages"
            onSuccess={() => {
              setIsEditing(false);
