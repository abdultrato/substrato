...
export default function TenantConfigurationsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/tenants/tenant-configurations/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["tenants", "tenant-configurations", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar TenantConfigurations</div>;
  if (!item) return <div>TenantConfigurations não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `TenantConfigurations #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar TenantConfigurations"
            onSuccess={() => {
              setIsEditing(false);
