...
export default function TenantFeatureFlagsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/tenants/tenant-feature-flags/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["tenants", "tenant-feature-flags", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar TenantFeatureFlags</div>;
  if (!item) return <div>TenantFeatureFlags não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `TenantFeatureFlags #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar TenantFeatureFlags"
            onSuccess={() => {
              setIsEditing(false);
