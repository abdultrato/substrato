...
export default function PasswordResetTokensDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/identity/password-reset-tokens/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["identity", "password-reset-tokens", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar PasswordResetTokens</div>;
  if (!item) return <div>PasswordResetTokens não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `PasswordResetTokens #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar PasswordResetTokens"
            onSuccess={() => {
              setIsEditing(false);
