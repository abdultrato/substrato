...
export default function DisciplineScheduleItemsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/education/discipline-schedule-items/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["education", "discipline-schedule-items", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar DisciplineScheduleItems</div>;
  if (!item) return <div>DisciplineScheduleItems não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `DisciplineScheduleItems #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar DisciplineScheduleItems"
            onSuccess={() => {
              setIsEditing(false);
