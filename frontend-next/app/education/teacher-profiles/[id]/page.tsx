...
export default function TeacherProfilesDetailPage() {
  ...
  const endpointBase = "/education/teacher-profiles/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["education", "teacher-profiles", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar TeacherProfiles</div>;
  if (!item) return <div>TeacherProfiles não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `TeacherProfiles #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar TeacherProfiles"
            onSuccess={() => {
              setIsEditing(false);
