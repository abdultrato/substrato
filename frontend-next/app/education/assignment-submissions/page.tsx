...
export default function EducationAssignmentSubmissionsPage() {
  ...
  const canCreate = hasWriteContract("/education/assignment-submissions/", "post");
  const canEdit = hasWriteContract("/education/assignment-submissions/{id}/", "put");
  const pageSize = 20;

  const { data, isLoading, error } = useQuery({
    queryKey: ["education", "assignment-submissions", page, search],
    queryFn: async () => {
      return await apiFetchList<any>("/education/assignment-submissions/", { page, pageSize, query: { search: search || undefined } });
    },
