...
export default function AssignmentSubmissionsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["education", "assignment-submissions", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AssignmentSubmissions #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar AssignmentSubmissions"
            onSuccess={() => {
              setIsEditing(false);
