...
export default function ExaminationAttemptsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/education/examination-attempts/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["education", "examination-attempts", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ExaminationAttempts</div>;
  if (!item) return <div>ExaminationAttempts não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ExaminationAttempts #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ExaminationAttempts"
            onSuccess={() => {
              setIsEditing(false);
