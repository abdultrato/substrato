...
export default function LearningContentsDetailPage() {
  useAuthGuard();
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["education", "learning-contents", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `LearningContents #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar LearningContents"
            onSuccess={() => {
              setIsEditing(false);
