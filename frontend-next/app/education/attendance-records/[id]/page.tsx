...
export default function AttendanceRecordsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/education/attendance-records/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["education", "attendance-records", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar AttendanceRecords</div>;
  if (!item) return <div>AttendanceRecords não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AttendanceRecords #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar AttendanceRecords"
            onSuccess={() => {
              setIsEditing(false);
