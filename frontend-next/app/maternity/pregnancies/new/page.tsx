...
export default function MaternityPregnanciesCreatePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await apiFetch<{ id: number }>("/maternity/gestacao/", {
        method: "POST",
        body: JSON.stringify(body),
        headers: { "Content-Type": "application/json" },
      });
      router.push(`/maternity/pregnancies/${res.id}`);
    } catch (e: any) {
      setError(e?.message || "Erro ao criar gestação.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <section className={`relative overflow-hidden ${GLASS}`}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 pl-5">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div>
                <h1 className="text-lg font-bold leading-tight text-foreground">Nova gestação</h1>
                <p className="text-[11px] text-muted-foreground">Registo de acompanhamento de gravidez</p>
              </div>
            </div>
