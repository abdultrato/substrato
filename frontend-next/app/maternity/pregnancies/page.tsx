...
type GestacaoRow = Record<string, any>
...
export default function MaternidadeGestacoesPage() {
    const { user } = useAuth()
    const podeVerAdmin = userHasAnyGroup(user, [GROUPS.ADMIN])

    const [erro, setErro] = useState<string | null>(null)
    const [loading, setLoading] = useState(true)
    const [data, setData] = useState<GestacaoRow[]>([])
        ...
        () => [
            {
                ...
                render: (g: GestacaoRow) => (
                    <Link
                        href={`/resources/maternity/gestacao/${g.id}`}
            ...
            { header: "Paciente", render: (g: GestacaoRow) => g.paciente_nome || "-" },
            { header: "Médico", render: (g: GestacaoRow) => g.medico_nome || "-" },
            { header: "Estado", render: (g: GestacaoRow) => g.estado || "-" },
            { header: "DPP", render: (g: GestacaoRow) => fmtDate(g.data_prevista_parto) },
        ],
        []
    )
    ...
    return (
        <AppLayout requiredGroups={[GROUPS.ADMIN, GROUPS.MEDICINA, GROUPS.MEDICINA_OCUPACIONAL]}>
            <div className="space-y-6">
                <PageHeader
                    title="Gestações"
                    subtitle="Registos de acompanhamento (maternidade)."
                    actions={
                ...
                {loading ? (
                    ...
                    <>
                        <DataTable<GestacaoRow>
                            columns={columns as any}
                            data={data}
                            emptyMessage="Nenhuma gestação encontrada."
                        />
