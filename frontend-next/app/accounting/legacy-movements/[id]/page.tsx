...
export default function LegacyMovementsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/accounting/legacy-movements/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["accounting", "legacy-movements", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar LegacyMovements</div>;
  if (!item) return <div>LegacyMovements não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `LegacyMovements #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar LegacyMovements"
            onSuccess={() => {
              setIsEditing(false);
