...
export default function LegacyEntriesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/accounting/legacy-entries/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["accounting", "legacy-entries", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar LegacyEntries</div>;
  if (!item) return <div>LegacyEntries não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `LegacyEntries #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar LegacyEntries"
            onSuccess={() => {
              setIsEditing(false);
