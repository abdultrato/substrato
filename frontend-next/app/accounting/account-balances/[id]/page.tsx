...
export default function AccountBalancesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/accounting/account-balances/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["accounting", "account-balances", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar AccountBalances</div>;
  if (!item) return <div>AccountBalances não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `AccountBalances #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar AccountBalances"
            onSuccess={() => {
              setIsEditing(false);
