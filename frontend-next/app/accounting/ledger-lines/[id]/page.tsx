...
export default function LedgerLinesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/accounting/ledger-lines/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["accounting", "ledger-lines", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar LedgerLines</div>;
  if (!item) return <div>LedgerLines não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `LedgerLines #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar LedgerLines"
            onSuccess={() => {
              setIsEditing(false);
