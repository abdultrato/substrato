...
export default function IntegrationDocumentsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/equipment_integrations/integration-documents/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["equipment_integrations", "integration-documents", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar IntegrationDocuments</div>;
  if (!item) return <div>IntegrationDocuments não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `IntegrationDocuments #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar IntegrationDocuments"
            onSuccess={() => {
              setIsEditing(false);
