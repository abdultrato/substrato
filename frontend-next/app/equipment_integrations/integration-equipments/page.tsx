...
export default function Equipment_IntegrationsIntegrationEquipmentsPage() {
  ...
  const canCreate = hasWriteContract("/equipment_integrations/integration-equipments/", "post");
  const canEdit = hasWriteContract("/equipment_integrations/integration-equipments/{id}/", "put");
  const pageSize = 20;

  const { data, isLoading, error } = useQuery({
    queryKey: ["equipment_integrations", "integration-equipments", page, search],
    queryFn: async () => {
      return await apiFetchList<any>("/equipment_integrations/integration-equipments/", { page, pageSize, query: { search: search || undefined } });
    },
