...
export default function IntegrationEquipmentsDetailPage() {
  useAuthGuard();
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["equipment_integrations", "integration-equipments", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `IntegrationEquipments #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar IntegrationEquipments"
            onSuccess={() => {
              setIsEditing(false);
