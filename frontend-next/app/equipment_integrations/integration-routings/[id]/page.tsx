...
export default function IntegrationRoutingsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/equipment_integrations/integration-routings/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["equipment_integrations", "integration-routings", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar IntegrationRoutings</div>;
  if (!item) return <div>IntegrationRoutings não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `IntegrationRoutings #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar IntegrationRoutings"
            onSuccess={() => {
              setIsEditing(false);
