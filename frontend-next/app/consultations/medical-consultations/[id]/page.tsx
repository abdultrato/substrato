...
export default function MedicalConsultationsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/consultations/medical-consultations/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["consultations", "medical-consultations", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar MedicalConsultations</div>;
  if (!item) return <div>MedicalConsultations não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `MedicalConsultations #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar MedicalConsultations"
            onSuccess={() => {
              setIsEditing(false);
