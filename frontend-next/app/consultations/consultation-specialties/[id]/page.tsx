...
export default function ConsultationSpecialtiesDetailPage() {
  ...
  const endpointBase = "/consultations/consultation-specialties/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["consultations", "consultation-specialties", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ConsultationSpecialties</div>;
  if (!item) return <div>ConsultationSpecialties não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ConsultationSpecialties #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ConsultationSpecialties"
            onSuccess={() => {
              setIsEditing(false);
