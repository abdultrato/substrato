...
export default function HolidaiesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/consultations/holidaies/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["consultations", "holidaies", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar Holidaies</div>;
  if (!item) return <div>Holidaies não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `Holidaies #${id}`}</h1>
