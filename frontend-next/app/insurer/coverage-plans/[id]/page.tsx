...
export default function CoveragePlansDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/insurer/coverage-plans/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["insurer", "coverage-plans", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar CoveragePlans</div>;
  if (!item) return <div>CoveragePlans não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `CoveragePlans #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar CoveragePlans"
            onSuccess={() => {
              setIsEditing(false);
