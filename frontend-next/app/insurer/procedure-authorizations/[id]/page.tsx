...
export default function ProcedureAuthorizationsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["insurer", "procedure-authorizations", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ProcedureAuthorizations #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ProcedureAuthorizations"
            onSuccess={() => {
              setIsEditing(false);
