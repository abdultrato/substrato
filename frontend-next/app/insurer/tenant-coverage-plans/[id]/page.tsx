...
export default function TenantCoveragePlansDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/insurer/tenant-coverage-plans/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["insurer", "tenant-coverage-plans", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar TenantCoveragePlans</div>;
  if (!item) return <div>TenantCoveragePlans não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `TenantCoveragePlans #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar TenantCoveragePlans"
            onSuccess={() => {
              setIsEditing(false);
