...
export default function PaymentsTransactionsPage() {
  ...
  const canCreate = hasWriteContract("/payments/transactions/", "post");
  const canEdit = hasWriteContract("/payments/transactions/{id}/", "put");
  const pageSize = 20;

  const { data, isLoading, error } = useQuery({
    queryKey: ["payments", "transactions", page, search],
    queryFn: async () => {
      return await apiFetchList<any>("/payments/transactions/", { page, pageSize, query: { search: search || undefined } });
    },
