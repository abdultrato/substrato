...
export default function ReconciliationsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/payments/reconciliations/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["payments", "reconciliations", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar Reconciliations</div>;
  if (!item) return <div>Reconciliations não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `Reconciliations #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar Reconciliations"
            onSuccess={() => {
              setIsEditing(false);
