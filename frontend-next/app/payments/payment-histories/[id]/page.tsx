...
export default function PaymentHistoriesDetailPage() {
  useAuthGuard();
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["payments", "payment-histories", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `PaymentHistories #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar PaymentHistories"
            onSuccess={() => {
              setIsEditing(false);
