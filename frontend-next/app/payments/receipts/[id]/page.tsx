...
export default function ReceiptsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/payments/receipts/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["payments", "receipts", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar Receipts</div>;
  if (!item) return <div>Receipts não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `Receipts #${id}`}</h1>
