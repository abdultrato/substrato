...
export default function MedicalExamFieldsDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["clinical", "medical-exam-fields", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `MedicalExamFields #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar MedicalExamFields"
            onSuccess={() => {
              setIsEditing(false);
