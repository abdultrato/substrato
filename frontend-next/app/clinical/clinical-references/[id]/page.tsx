...
export default function ClinicalReferencesDetailPage() {
  ...
  const { data: item, isLoading, error } = useQuery({
    queryKey: ["clinical", "clinical-references", id],
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ClinicalReferences #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ClinicalReferences"
            onSuccess={() => {
              setIsEditing(false);
