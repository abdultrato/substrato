...
export default function MedicalResultFilesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/clinical/medical-result-files/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["clinical", "medical-result-files", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar MedicalResultFiles</div>;
  if (!item) return <div>MedicalResultFiles não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `MedicalResultFiles #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar MedicalResultFiles"
            onSuccess={() => {
              setIsEditing(false);
