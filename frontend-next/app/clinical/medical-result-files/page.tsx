...
export default function ClinicalMedicalResultFilesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      return await apiFetchList<any>("/api/v1/clinical/medical-result-files/?page={page}&search={search}");
    },
              ...
              {data?.items.map((item: any) => (
                <tr key={item.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 space-x-2">
                    <Link
                      href={`./medical-result-files/${item.id}`}
                      className="text-blue-600 hover:underline"
                    >
                      Ver
                    </Link>
                    <button
                      onClick={() => window.location.href = `./medical-result-files/${item.id}/edit`}
