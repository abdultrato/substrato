...
export default function ClinicalHistoriesDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/clinical/clinical-histories/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["clinical", "clinical-histories", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ClinicalHistories</div>;
  if (!item) return <div>ClinicalHistories não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ClinicalHistories #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ClinicalHistories"
            onSuccess={() => {
              setIsEditing(false);
