...
export default function ResultItemsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/clinical/result-items/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["clinical", "result-items", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ResultItems</div>;
  if (!item) return <div>ResultItems não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ResultItems #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ResultItems"
            onSuccess={() => {
              setIsEditing(false);
