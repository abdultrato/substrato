...
export default function ClinicalEventsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/clinical/clinical-events/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["clinical", "clinical-events", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar ClinicalEvents</div>;
  if (!item) return <div>ClinicalEvents não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `ClinicalEvents #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar ClinicalEvents"
            onSuccess={() => {
              setIsEditing(false);
