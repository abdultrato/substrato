...
export default function LabRequestItemsDetailPage() {
  useAuthGuard();
  ...
  const endpointBase = "/clinical/lab-request-items/";
  const canEdit = hasWriteContract(`${endpointBase}{id}/`, "put");

  const { data: item, isLoading, error } = useQuery({
    queryKey: ["clinical", "lab-request-items", id],
    queryFn: async () => await apiFetch(`${endpointBase}${id}/`),
  });

  if (isLoading) return <div>Carregando...</div>;
  if (error) return <div>Erro ao carregar LabRequestItems</div>;
  if (!item) return <div>LabRequestItems não encontrado</div>;

  return (
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{item.name || item.title || `LabRequestItems #${id}`}</h1>
        ...
        {isEditing && canEdit ? (
          <AutoForm
            ...
            method="put"
            initialValues={item}
            submitLabel="Salvar LabRequestItems"
            onSuccess={() => {
              setIsEditing(false);
