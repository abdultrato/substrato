...
#
# Uso:
#   ./scripts/delete_user_rbac.sh               # modo interativo
#   ./scripts/delete_user_rbac.sh -u joao       # desativar (com confirmação)
#   ./scripts/delete_user_rbac.sh -u joao --hard --yes
...
Uso:
  scripts/delete_user_rbac.sh                       (modo interativo)
  scripts/delete_user_rbac.sh -u USERNAME [opções]
  scripts/delete_user_rbac.sh --email EMAIL [opções]
...
Exemplos:
  scripts/delete_user_rbac.sh
  scripts/delete_user_rbac.sh -u recep1
  scripts/delete_user_rbac.sh -u recep1 --hard --yes
EOF
}
...
if [[ ${#ARGS[@]} -eq 0 ]]; then
  ...
  if [[ "$hard" == "s" || "$hard" == "sim" || "$hard" == "y" || "$hard" == "yes" ]]; then
    ...
    run_manage delete_user_rbac --username "$username" --hard --yes
  else
    echo
    ...
    run_manage delete_user_rbac --username "$username" --yes
  fi
...
run_manage delete_user_rbac "${ARGS[@]}"
