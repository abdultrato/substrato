...
#
# Exemplos:
#   ./scripts/create_user_rbac.sh --list-groups
#   ./scripts/create_user_rbac.sh -u joao -p 'SenhaForte123' -n 'João Paulo' -g MEDICINA --exclusive --reset-password
#
# Notas:
...
Uso:
  scripts/create_user_rbac.sh --list-groups
  scripts/create_user_rbac.sh --wizard
  scripts/create_user_rbac.sh -u USERNAME -p PASSWORD -n "NOME COMPLETO" -g GRUPO [opções]
...
Exemplos:
  scripts/create_user_rbac.sh --list-groups
  scripts/create_user_rbac.sh -u admin -p 'admin123' -n 'Administrador' -g ADMIN --reset-password --exclusive
  scripts/create_user_rbac.sh -u recep1 -p 'Recep#2026' -n 'Maria da Silva' -g RECEPCAO --exclusive --reset-password
EOF
}
...
run_manage create_user_rbac "${ARGS[@]}"
