...
#   ./scripts/reset_banco_e_migracoes.sh --no-backup
#   ./scripts/reset_banco_e_migracoes.sh --docker-db   # força reset do Postgres via docker compose (serviço "db")
#
# Segurança:
...
print_usage() {
  cat <<'USAGE'
Uso: scripts/reset_banco_e_migracoes.sh [opções]
