...
from infrastructure.context.tenant import (
    reset_inquilino,
    set_inquilino,
)
...
class InquilinoMiddleware:
    ...
    def _handle(self, request):
        ...
        # criação/validação de models que exigem `inquilino` via InquilinoMixin.
        ...
        if settings.DEBUG:
            ...
            try:
                if host:
                    inquilino = Tenant.objects.filter(dominio=host).order_by("id").first()

                if not inquilino:
                    inquilino = Tenant.objects.filter(ativo=True).order_by("id").first()
                ...
                if not inquilino:
                    ...
                    try:
                        ...
                        inquilino = Tenant.objects.filter(identificador="local").order_by("id").first()
            except OperationalError:
                return JsonResponse({"erro": "Base de dados indisponível."}, status=503)
        ...
        if not inquilino and request.path.startswith("/api/v1/equipment_integrations/equipment/"):
            try:
                from apps.equipment_integrations.models.credential import (
    ...
    def _resolver_inquilino(self, host):
        ...
        if tenant_id:
            inquilino = Tenant.objects.only("id", "ativo").filter(id=tenant_id, ativo=True).first()
        ...
        inquilino = Tenant.objects.only("id", "ativo").filter(dominio=host, ativo=True).first()
