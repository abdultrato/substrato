...
class RazaoService:
    ...
    def saldo_por_conta(inquilino, data_inicio=None, data_fim=None):
        ...
        if data_inicio:
            queryset = queryset.filter(lancamento__data_contabil__gte=data_inicio)

        if data_fim:
            queryset = queryset.filter(lancamento__data_contabil__lte=data_fim)

        return (
            queryset
            .values("conta__id", "conta__nome", "conta__codigo")
            .annotate(
