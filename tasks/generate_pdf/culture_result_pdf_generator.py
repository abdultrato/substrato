...
def _estado_linhas(culture: MicrobiologyCulture) -> list[str]:
    linhas = [f"{bold_text('Estado')}: {_safe_text(culture.get_status_display())}"]
    if culture.incubation_started_at:
        linhas.append(f"{bold_text('Início da incubação')}: {_safe_text(_format_datetime(timezone.localtime(culture.incubation_started_at)))}")
    if culture.incubation_expected_end_at:
        linhas.append(f"{bold_text('Leitura prevista')}: {_safe_text(_format_datetime(timezone.localtime(culture.incubation_expected_end_at)))}")
    if culture.incubation_accumulated_hours:
        linhas.append(f"{bold_text('Incubação acumulada')}: {_safe_text(f'{culture.incubation_accumulated_hours}h')}")
    return linhas


def _plate_table(culture: MicrobiologyCulture, usable_width, sector_color) -> Table:
    plates = culture.culture_plates or []
    rows = [[cell_paragraph("Meio", True), cell_paragraph("Resultado", True)]]
    if plates:
        for plate in plates:
            if not isinstance(plate, dict):
                continue
            is_positive = plate.get("outcome") in ("positive", "contaminated")
            rows.append([cell_paragraph(_plate_label(plate)), cell_paragraph(_plate_result(plate), is_positive)])
    else:
        rows.append([cell_paragraph("—"), cell_paragraph("Sem meios semeados")])
    table = Table(rows, colWidths=[usable_width * 0.42, usable_width * 0.58])
    table.setStyle(
        TableStyle(
            [
                ("FONTNAME", (0, 0), (-1, 0), FONT_BOLD_INST),
                ...
                ("RIGHTPADDING", (0, 0), (-1, -1), 3),
                ("BOTTOMPADDING", (0, 0), (-1, -1), 4),
                ("LINEBELOW", (0, 0), (-1, 0), 0.5, sector_color),
            ]
        )
    )
    return table
...
def generate_cultures_by_order_pdf(cultures, request=None) -> tuple[bytes, str]:
    """Laudo institucional único com o resultado de TODAS as culturas de uma
    requisição, com o estado e timers exatos de cada meio no momento."""
    cultures = [c for c in cultures if c is not None]
    if not cultures:
        raise ValueError("Nenhuma cultura para imprimir.")
    ...
    patient = order.patient
    tenant = getattr(patient, "tenant", None) or getattr(first, "tenant", None)
    tenant_name = getattr(tenant, "name", "CLÍNICA DE DIAGNÓSTICOS E SAÚDE")
    ...
    doc.barcode_value = f"PAC:{getattr(patient, 'custom_id', '')}|REQ:{order.custom_id}"

    technician_text = institutional_user_identity(first.performed_by or getattr(order, "analyst", None))
    now = timezone.localtime()
    ...
    left_lines = [
        ...
        f"{bold_text('Documento')}: {_safe_text(getattr(patient, 'document_type', '—'))}: {_safe_text(getattr(patient, 'document_number', '') or 'Não forneceu documento')}",
        f"{bold_text('Contacto')}: {_safe_text(getattr(patient, 'contact', '') or '—')}",
        f"{bold_text('Culturas')}: {_safe_text(len(cultures))}",
    ]
    right_lines = [
        f"{bold_text('Requisição')}: {_safe_text(order.custom_id)}",
        f"{bold_text('Emitido em')}: {_safe_text(_format_datetime(now))}",
    ...
    elements = [
        Paragraph("RESULTADOS DE CULTURA", title_style_improved()),
        Spacer(1, A5Margins.SECTION_SPACING),
        montar_bloco_identificacao(usable_width=usable_width, left_lines=left_lines, right_lines=right_lines),
        Spacer(1, A5Margins.SECTION_SPACING),
        HRFlowable(width="100%", thickness=0.5, color=sector_color),
        Spacer(1, A5Margins.SECTION_SPACING),
    ]

    for idx, culture in enumerate(cultures):
        if idx > 0:
            elements.extend([Spacer(1, A5Margins.SECTION_SPACING), HRFlowable(width="100%", thickness=0.3, color=sector_color), Spacer(1, A5Margins.ROW_SPACING)])
        elements.append(Paragraph(escape(_exam_name(culture).upper()), section_style_improved(color=sector_color)))
        elements.append(Spacer(1, A5Margins.ROW_SPACING))
        for linha in _estado_linhas(culture):
            elements.append(cell_paragraph(linha))
        elements.append(Spacer(1, A5Margins.ROW_SPACING))
        elements.append(_plate_table(culture, usable_width, sector_color))
        if (culture.notes or "").strip():
            elements.append(Spacer(1, A5Margins.ROW_SPACING))
            elements.append(cell_paragraph(f"{bold_text('Observações')}: {_safe_text(culture.notes)}"))

    append_fim(elements)

    first_frame = Frame(
        A5Margins.LEFT, A5Margins.BOTTOM, usable_width, page_height - A5Margins.TOP - A5Margins.BOTTOM,
        id="first", leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0,
    )
    later_frame = Frame(
        A5Margins.LEFT, A5Margins.BOTTOM, usable_width, page_height - PDF_MARGIN - A5Margins.BOTTOM,
    ...
    pdf_bytes = buffer.getvalue()
    buffer.close()
    filename = f"culturas_{order.custom_id}.pdf".replace("/", "-")
    return pdf_bytes, filename


gerar_pdf_cultura = generate_culture_result_pdf
gerar_pdf_culturas_requisicao = generate_cultures_by_order_pdf
