...
class PDFAdminMixin:
    ...
    def download_pdf_view(self, request: HttpRequest, pk: int) -> HttpResponse:
        ...
        try:
            ...
            pdf_bytes, filename = self.pdf_generator(obj, request=request)
    ...
    def download_pdf_action(self, request: HttpRequest, queryset):
        ...
        try:
            ...
            logger.exception(f"Erro na ação de PDF")
            self.message_user(request, f"Erro ao gerar PDF: {str(e)}", messages.ERROR)
    
    download_pdf_action.short_description = "Baixar PDF"
    
    @classmethod
    def register_pdf_generator(cls, generator_func: Callable, filename_template: str = None):
